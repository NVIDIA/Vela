...
test("vsr module exists", function()
  assert(vsr ~= nil, "vsr module should exist")
end)

-- Vector types
section("Vector Types")

test("vsr.float2(x, y)", function()
  local v = vsr.float2(1.0, 2.0)
  assert(v ~= nil, "float2 should be created")
  assert(v.x == 1.0, "v.x should be 1.0")
  assert(v.y == 2.0, "v.y should be 2.0")
end)

test("vsr.float2()", function()
  local v = vsr.float2()
  assert(v ~= nil, "float2 should be created")
...
test("vsr.float3(x, y, z)", function()
  local v = vsr.float3(1.0, 2.0, 3.0)
  assert(v ~= nil, "float3 should be created")
  assert(v.x == 1.0, "v.x should be 1.0")
  assert(v.y == 2.0, "v.y should be 2.0")
...
test("vsr.float3(v) rejects single-arg call", function()
...
test("vsr.float4(x, y, z, w)", function()
  local v = vsr.float4(1.0, 2.0, 3.0, 4.0)
  assert(v ~= nil, "float4 should be created")
  assert(v.x == 1.0, "v.x should be 1.0")
  assert(v.w == 4.0, "v.w should be 4.0")
end)

test("vsr.float4(float3, w) rejects mixed-arg call", function()
  local ok = pcall(function()
    vsr.float4(vsr.float3(1.0, 2.0, 3.0), 4.0)
  end)
  assert(not ok, "float4(float3, w) should not be supported")
end)

test("vsr.float4(v) rejects single-arg call", function()
  local ok = pcall(function()
    vsr.float4(5.0)
  end)
...
test("vsr.length(float3)", function()
  local v = vsr.float3(3.0, 4.0, 0.0)
  local len = vsr.length(v)
  assert(math.abs(len - 5.0) < 0.001, "length should be 5.0")
end)

test("vsr.normalize(float3)", function()
  local v = vsr.float3(3.0, 4.0, 0.0)
  local n = vsr.normalize(v)
  assert(n ~= nil, "normalized vector should exist")
  local len = vsr.length(n)
  assert(math.abs(len - 1.0) < 0.001, "normalized length should be 1.0")
end)

test("vsr.dot(float3, float3)", function()
  local a = vsr.float3(1.0, 0.0, 0.0)
  local b = vsr.float3(0.0, 1.0, 0.0)
  local d = vsr.dot(a, b)
  assert(math.abs(d) < 0.001, "dot product of perpendicular vectors should be 0")
end)

test("vsr.cross(float3, float3)", function()
  local a = vsr.float3(1.0, 0.0, 0.0)
  local b = vsr.float3(0.0, 1.0, 0.0)
  local c = vsr.cross(a, b)
  assert(c ~= nil, "cross product should exist")
  assert(math.abs(c.z - 1.0) < 0.001, "cross product z should be 1.0")
end)

test("vsr.radians(degrees)", function()
  local r = vsr.radians(180.0)
  assert(math.abs(r - math.pi) < 0.001, "180 degrees should be pi radians")
end)

test("vsr.degrees(radians)", function()
  local d = vsr.degrees(math.pi)
...
test("vsr.translation(float3)", function()
  local t = vsr.translation(vsr.float3(1.0, 2.0, 3.0))
  assert(t ~= nil, "translation matrix should be created")
end)

test("vsr.scaling(float3)", function()
  local s = vsr.scaling(vsr.float3(2.0, 2.0, 2.0))
  assert(s ~= nil, "scaling matrix should be created")
end)

test("vsr.scaling(float)", function()
  local s = vsr.scaling(2.0)
  assert(s ~= nil, "uniform scaling matrix should be created")
end)

test("vsr.rotation(axis, angle)", function()
  local r = vsr.rotation(vsr.float3(0.0, 1.0, 0.0), vsr.radians(90.0))
  assert(r ~= nil, "rotation matrix should be created")
end)

test("matrix multiplication", function()
  local t = vsr.translation(vsr.float3(1.0, 0.0, 0.0))
  local s = vsr.scaling(2.0)
  local combined = t * s
...
test("matrix-vector multiplication", function()
  local v = vsr.float4(1.0, 2.0, 3.0, 1.0)
  local out = vsr.mat4.identity * v
...
test("ref:setParameter with float3", function()
  local mat = scene:createMaterial("matte")
  mat:setParameter("color", vsr.float3(1.0, 0.0, 0.0))
...
test("ref:type()", function()
  local geom = scene:createGeometry("sphere")
  assert(geom:type() == vsr.GEOMETRY, "type should be GEOMETRY")
  local mat = scene:createMaterial("matte")
  assert(mat:type() == vsr.MATERIAL, "type should be MATERIAL")
  local light = scene:createLight("point")
  assert(light:type() == vsr.LIGHT, "type should be LIGHT")
end)
...
test("array:setData accepts float3 userdata entries", function()
  ...
  arr:setData({
    vsr.float3(0.0, 1.0, 2.0),
    vsr.float3(3.0, 4.0, 5.0),
  })
...
test("vsr.io module and procedural generators exist", function()
  assert(vsr.io ~= nil, "vsr.io module should exist")
  local generators = {
    "generateRandomSpheres", "generateMonkey", "generateCylinders",
    "generateIcosphere", "generateDefaultLights", "generateHdriDome",
    "generateRtow",
  }
  for _, name in ipairs(generators) do
    assert(vsr.io[name] ~= nil, name .. " should exist")
  end
end)
...
test("vsr.io import functions exist", function()
...
test("Create scene with transform", function()
  ...
  local mat = scene:createMaterial("physicallyBased")
  mat:setParameter("baseColor", vsr.float3(0.2, 0.5, 0.8))
  mat:setParameter("metallic", 0.5)
  mat:setParameter("roughness", 0.3)

  local surface = scene:createSurface("pbrSphere", geom, mat)

  local xfm = vsr.translation(vsr.float3(2.0, 0.0, 0.0)) * vsr.scaling(0.5)
...
test("createMaterial with param table", function()
  local mat = scene:createMaterial("matte", { color = vsr.float3(1.0, 0.0, 0.0) })
...
test("param table with multiple params", function()
  local mat = scene:createMaterial("physicallyBased", {
    baseColor = vsr.float3(0.2, 0.5, 0.8),
    metallic = 0.5,
    roughness = 0.3,
  })
  assert(mat:valid(), "material should be valid")
...
test("ANARI type constants exist and are unique", function()
  ...
  for _, name in ipairs(names) do
    assert(vsr[name] ~= nil, "vsr." .. name .. " should exist")
    local value = vsr[name]
    assert(not seen[value], "vsr." .. name .. " should be unique")
    seen[value] = true
  end
  -- Verify they can be used with numberOfObjects (functional test)
  local s = vsr.createScene()
  local before = s:numberOfObjects(vsr.GEOMETRY)
  s:createGeometry("sphere")
  local after = s:numberOfObjects(vsr.GEOMETRY)
...
test("float3 addition", function()
  local a = vsr.float3(1, 2, 3)
  local b = vsr.float3(4, 5, 6)
  local c = a + b
...
test("float3 subtraction", function()
  local a = vsr.float3(4, 5, 6)
  local b = vsr.float3(1, 2, 3)
  local c = a - b
...
test("float2 addition", function()
  local a = vsr.float2(1, 2)
  local b = vsr.float2(3, 4)
  local c = a + b
...
test("float2 subtraction", function()
  local a = vsr.float2(4, 5)
  local b = vsr.float2(1, 2)
  local c = a - b
...
test("float4 addition", function()
  local a = vsr.float4(1, 2, 3, 4)
  local b = vsr.float4(5, 6, 7, 8)
  local c = a + b
...
test("float4 subtraction", function()
  local a = vsr.float4(5, 6, 7, 8)
  local b = vsr.float4(1, 2, 3, 4)
  local c = a - b
...
test("mat3(float3, float3, float3) constructor", function()
  local c0 = vsr.float3(1.0, 2.0, 3.0)
  local c1 = vsr.float3(4.0, 5.0, 6.0)
  local c2 = vsr.float3(7.0, 8.0, 9.0)
  local m = vsr.mat3(c0, c1, c2)
...
test("mat4(float4, float4, float4, float4) constructor", function()
  local c0 = vsr.float4(1.0, 0.0, 0.0, 0.0)
  local c1 = vsr.float4(0.0, 1.0, 0.0, 0.0)
  local c2 = vsr.float4(0.0, 0.0, 1.0, 0.0)
  local c3 = vsr.float4(10.0, 20.0, 30.0, 1.0)
  local m = vsr.mat4(c0, c1, c2, c3)
...
test("scene:numberOfObjects(type)", function()
  ...
  s:createGeometry("sphere")
  s:createGeometry("sphere")
  local n = s:numberOfObjects(vsr.GEOMETRY)
...
test("scene:removeAllObjects()", function()
  ...
  s:createMaterial("matte")
  s:removeAllObjects()
  assert(s:numberOfObjects(vsr.GEOMETRY) == 0, "should have 0 geometries")
  assert(s:numberOfObjects(vsr.MATERIAL) == 0, "should have 0 materials")
end)
...
test("scene:removeObject(nil) is a no-op", function()
  ...
  s:createGeometry("sphere")
  s:removeObject(nil)
  assert(s:numberOfObjects(vsr.GEOMETRY) == 1,
...
test("Layer:foreach provides LayerNodeRef with full API", function()
  ...
  s:insertChildTransformNode(root, vsr.mat4.identity, "childXfm")
...
test("insertChildTransformNode", function()
  local layer = scene:defaultLayer()
  local root = layer:root()
  local xfm = vsr.translation(vsr.float3(1, 2, 3))
...
test("vsr.io.generateSphereSetVolume exists", function()
  assert(vsr.io.generateSphereSetVolume ~= nil, "generateSphereSetVolume should exist")
end)
...
test("vsr.render.createRenderIndex rejects nil device", function()
  local ok, err = pcall(function()
    vsr.render.createRenderIndex(scene, nil)
  end)
  assert(not ok, "createRenderIndex(nil) should fail")
  assert(err ~= nil, "error message should be present")
end)

test("vsr.render.getWorldBounds rejects nil handles", function()
  local ok, err = pcall(function()
    vsr.render.getWorldBounds(nil, nil)
  end)
  assert(not ok, "getWorldBounds(nil, nil) should fail")
  assert(err ~= nil, "error message should be present")
end)

test("vsr.render.renderToFile rejects nil pipeline", function()
  local ok, err = pcall(function()
    vsr.render.renderToFile(nil, 1, "out.ppm", 64, 64)
  end)
