...
local function usage()
    print("Usage: vsrLua save_scene_archive.lua [options]")
    print("")
    print("Options:")
...
print("==========================")
print("Scene Archive:    " .. outPrefix .. ".vsr")
print("Animation Archive: " .. outPrefix .. "_animations.vsr")
...
vsr.io.generateHdriDome(scene)


local layer = scene:defaultLayer()
local rootXfm = scene:insertChildTransformNode(layer:root(), vsr.mat4.identity, "ROOT")

print("Generating monkey mesh...")
local monkeyXfm = scene:insertChildTransformNode(rootXfm, vsr.mat4.identity, "Monkey")
vsr.io.generateMonkey(scene, monkeyXfm)
...
for row = 0, gridRows - 1 do
    for col = 0, gridCols - 1 do
        ...
        xfmData[row * gridCols + col + 1] = vsr.translation(vsr.float3(x, 0, z))
    end
end
...
local orbitCenter = vsr.float3(0, 0, 0)
...
camera:setParameter("up", vsr.float3(0, 1, 0))
...
for frame = 0, numFrames - 1 do
    ...
    local pos = orbitCenter + vsr.float3(orbitDist * sx, orbitHeight, orbitDist * cz)
    local dir = vsr.normalize(orbitCenter - pos)
...
local sceneFilename = outPrefix .. ".vsr"
local animationFilename = outPrefix .. "_animations.vsr"
print("Saving Scene Archive to " .. sceneFilename .. "...")
vsr.io.saveSceneArchive(scene, sceneFilename)
print("Saving Animation Manager Archive to " .. animationFilename .. "...")
vsr.io.saveAnimationManagerArchive(animationMgr, animationFilename)
