...
local function usage()
    print("Usage: vsrLua render_rtow_hdri.lua [options]")
    print("")
    print("Options:")
...
local rtowXfm = scene:insertChildTransformNode(rootXfm, vsr.mat4.identity, "RTOW")
vsr.io.generateRtow(scene, rtowXfm)
...
print("Generate icosphere scene")
local isoXfm = scene:insertChildTransformNode(rootXfm, vsr.scaling(8), "Icosphere")
vsr.io.generateIcosphere(scene, isoXfm)

print("Loading ANARI device...")
local device = vsr.render.loadDevice(libraryName)

print("Creating render index...")
local renderIndex = vsr.render.createRenderIndex(scene, device)
renderIndex:populate()

print("Computing scene bounds...")
local bounds = vsr.render.getWorldBounds(device, renderIndex)
local center = (bounds.min + bounds.max) * 0.5
local diag = bounds.max - bounds.min
local dist = vsr.length(diag) / 3.0
if dist < 1e-3 then
    dist = 5.0
end

local camera = vsr.CameraSetup.new()
camera.position = center + vsr.float3(0, 10, -dist)
camera.direction = vsr.normalize(center - camera.position)
camera.up = vsr.float3(0, 1, 0)
camera.fovy = 40.0
