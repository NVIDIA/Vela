...
Two routes were investigated and both are closed at the level VSR can reach.

The device cannot resolve the tile set on VSR's behalf. VisRTX does load MDL
...
VSR cannot expand the tiles either, for the same reason. Rewriting the
...
"supporting UDIM in VSR" has nowhere to send the texels.
