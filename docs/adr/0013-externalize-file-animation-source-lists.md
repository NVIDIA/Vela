...
`datasets/<name>.sources` next to `datasets/<name>.vsr`. The dataset file
...
still scans only `.vsr` files. This revises ADR 0003/0004's rule that the
dataset `.vsr` contains its frame references and ADR 0005's rule that the
...
and rewrites the `.vsr` without paths, following the ADR 0006 pattern.
