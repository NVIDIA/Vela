...
time samples, the `anari:` and `vsr:io:` vocabularies — which is what it was
always retained for (ADR 0015).

Keying by path rather than by Scene or by AnimationManager keeps USD knowledge
out of `vsr_scene` and `vsr_animation`, which sit below `vsr_io`, and makes
...
The Session also owns the mapping from VSR's normalized animation time onto the
...
a VSR scene safe to use across threads while USD is being read. ADR 0018 already
