...
VSR imports USD stages by consuming a UsdImaging scene index chain rather than
...
more. The stage stays open alongside the scene index, so VSR-specific data that
Hydra does not model — `customData` carriers, `anari:` and `vsr:io:` attributes,
