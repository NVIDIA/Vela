...
VSR cannot fix the asset, but it no longer emits a document the device cannot
...
`color3`-into-`float` scenario in `src/tests/test_UsdImport_materials.cpp`.
...
Setting `VSR_USD_MATERIALX_DUMP_DIR` writes each generated document there,
...
and VSR never sets the device's `mdlResourceSearchPaths` parameter
...
after `anari::newDevice` in `vsrOffline`:
...
`SamplerRegistry::loadFromImage` binds it. The render visibly changes. VSR's own
...
the above. Unrelated to USD import, and in `devices/`, not VSR.

## Measuring

Reproducing any of this needs three things that are not in the repo: the
reference asset, a VSR build with `VSR_USE_USD=ON` and `VSR_USE_OIIO=ON` (the
...
VSR build is enough for VSR's own side; the device comes from the parent repo.

`vsrOffline` defaults to `VSR_ANARI_LIBRARIES[0]`, then the environment, then
`visrtx` -- in practice `visrtx`, which has no MaterialX shader generation and
so reports none of the failures above -- pass `--lib visrtx_mtlx` explicitly. It
also prompts for a camera:

```bash
F=.../ShdrPlygrnd/ShdrPlygrnd_OpenPBR.usda
echo 8 | ./vsrOffline --lib visrtx_mtlx -usd_mtlx $F -o /tmp/out.png -s 1 -w 128 -h 96 > /tmp/run.log 2>&1
```
...
`src/tests/test_UsdImport_materials.cpp` and `src/tests/test_Importers.cpp` carry
the fixture patterns, both hand-writing decodable 1x1 files because the decoders
need real ones.
