...
inline vsr::math::float3 lerpAzElDist(
    float t, const vsr::math::float3 &a, const vsr::math::float3 &b)
{
  ...
  return vsr::math::float3{azimuth, elevation, distance};
}
    ...
    float t)
{
  ...
  auto lerpVec3 =
      [&lerp](float t, const vsr::math::float3 &a, const vsr::math::float3 &b) {
...
inline void smoothCameraPath(std::vector<CameraPose> &poses, float smoothness)
{
  ...
  for (size_t i = 0; i < poses.size(); ++i) {
    vsr::math::float3 avgLookat(0.f);
    vsr::math::float3 avgAzElDist(0.f);
    ...
    for (int offset = -windowSize; offset <= windowSize; ++offset) {
      ...
      if (totalWeight == weight) {
        ...
        vsr::math::float3 normalized = poses[idx].azeldist;
