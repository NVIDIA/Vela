...
// vsr_rendering
#include "vsr/rendering/index/RenderIndexFilterFcn.hpp"
// std
#include <algorithm>
 ...
 */
struct TransformsToAnariVisitor : public vsr::scene::LayerVisitor
{
  ...
  bool preChildren_const(const vsr::scene::LayerNode &n, int level) override;
  void postChildren_const(const vsr::scene::LayerNode &n, int level) override;

 private:
  bool isIncludedAfterFiltering(const vsr::scene::LayerNode &n) const;

  anari::Device m_device{nullptr};
  anari::Instance *m_currentInstance;
  std::stack<vsr::math::mat4> m_xfms;
  std::stack<bool> m_hasObjects;
  const vsr::scene::Array *m_xfmArray{nullptr};
...
{
  anari::retain(d, d);
  m_xfms.emplace(vsr::math::identity);
...
inline bool TransformsToAnariVisitor::preChildren_const(
    const vsr::scene::LayerNode &n, int level)
{
  if (!n->isEnabled())
    return false;
  ...
  case ANARI_FLOAT32_MAT4:
    m_xfms.push(vsr::math::mul(m_xfms.top(), n->getTransform()));
    m_hasObjects.emplace(false);
    break;
  case ANARI_ARRAY1D: {
    ...
    const vsr::scene::LayerNode &n, int level)
{
  ...
  case ANARI_ARRAY1D: {
    ...
    if (m_hasObjects.top()) {
      anari::Instance instance = *m_currentInstance++;
      const auto *xfms_in = m_xfmArray->dataAs<vsr::math::mat4>();

      uint64_t stride = 0;
      auto *xfms_out = (vsr::math::mat4 *)anariMapParameterArray1D(m_device,
          instance,
          "transform",
          ANARI_FLOAT32_MAT4,
          m_xfmArray->size(),
      ...
      if (stride == sizeof(vsr::math::mat4)) {
        std::transform(xfms_in,
            xfms_in + m_xfmArray->size(),
            xfms_out,
            [&xfm = m_xfms.top()](
                const vsr::math::mat4 &m) { return vsr::math::mul(xfm, m); });
      } else {
...
inline bool TransformsToAnariVisitor::isIncludedAfterFiltering(
    const vsr::scene::LayerNode &n) const
{
  if (!m_filter)
    return true;
