...
void registerMathBindings(sol::state &lua)
{
  ...
  auto float4Type = vsr.new_usertype<math::float4>("float4",
  ...
  vsr.new_usertype<math::mat4>("mat4",
  ...
  vsr["mat3"]["identity"] = math::IDENTITY_MAT3;
  vsr["mat4"]["identity"] = math::IDENTITY_MAT4;
  vsr["srt"] = vsr["mat3"];
  ...
  vsr["cross"] = [](const math::float3 &a, const math::float3 &b) {
    return math::cross(a, b);
  };

  // Transform matrices
  vsr["translation"] = [](const math::float3 &t) {
    return math::translation_matrix(t);
  };

  vsr["scaling"] = sol::overload(
      [](const math::float3 &s) { return math::scaling_matrix(s); },
      [](float s) { return math::scaling_matrix(math::float3(s, s, s)); });

  vsr["rotation"] = [](const math::float3 &axis, float angle) {
    return math::rotation_matrix(math::rotation_quat(axis, angle));
  };

  vsr["radians"] = [](float degrees) { return math::radians(degrees); };
  vsr["degrees"] = [](float radians) { return math::degrees(radians); };
}
