...
void registerContextBindings(sol::state &lua)
{
  ...
  vsr.new_usertype<core::Token>("Token",
  ...
  vsr.new_usertype<scene::Parameter>(
      "Parameter",
      sol::no_constructor,
      "name",
  ...
  vsr.new_usertype<vsr::animation::Animation>(
      "Animation",
      sol::no_constructor,
      "name",
      &vsr::animation::Animation::name,
      "addObjectParameterBinding",
      [](vsr::animation::Animation &a,
          sol::object target,
        ...
        auto interp = vsr::animation::InterpolationRule::LINEAR;
        if (interpStr && *interpStr == "step")
          interp = vsr::animation::InterpolationRule::STEP;
        else if (interpStr && *interpStr == "slerp")
          interp = vsr::animation::InterpolationRule::SLERP;
  ...
  vsr["createScene"] = []() { return std::make_unique<scene::Scene>(); };

  // ANARI data type constants
  vsr["GEOMETRY"] = ANARI_GEOMETRY;
  vsr["MATERIAL"] = ANARI_MATERIAL;
  vsr["LIGHT"] = ANARI_LIGHT;
  vsr["CAMERA"] = ANARI_CAMERA;
  vsr["SURFACE"] = ANARI_SURFACE;
  vsr["VOLUME"] = ANARI_VOLUME;
  vsr["SAMPLER"] = ANARI_SAMPLER;
  vsr["ARRAY"] = ANARI_ARRAY;
  vsr["SPATIAL_FIELD"] = ANARI_SPATIAL_FIELD;
}

void registerAnimationManagerBindings(sol::state &lua)
{
  using SA = vsr::animation::AnimationManager;
  sol::table vsr = lua["vsr"];
  ...
  vsr.new_usertype<SA>(
      ...
      "addAnimation",
      sol::overload(
          [](SA &sa) -> vsr::animation::Animation & {
            return sa.addAnimation();
          },
          [](SA &sa, const std::string &name) -> vsr::animation::Animation & {
            return sa.addAnimation(name);
          }),
      "animations",
      [](SA &sa) -> std::vector<vsr::animation::Animation> & {
