...
ConsoleCommandResult LuaContext::runRegisteredCommand(const std::string &line)
{
  ...
  sol::table vsr = m_impl->lua["vsr"];
  if (!vsr.valid())
    return result;
  sol::optional<sol::table> terminal = vsr["terminal"];
  if (!terminal)
    return result;
  ...
  for (const auto &path : paths) {
    ...
    if (fs::exists(initFile) && fs::is_regular_file(initFile)) {
      ...
      if (!result.valid()) {
        ...
        vsr::core::logWarning("%s", msg.c_str());
...
void LuaContext::bindAnimationManager(
    vsr::animation::AnimationManager *sa, const std::string &varName)
{
...
std::vector<std::string> LuaContext::defaultSearchPaths()
{
...
#ifdef VSR_SOURCE_DIR
  try {
    fs::path sourcePath = VSR_SOURCE_DIR;
    fs::path scriptsPath = sourcePath / "scripts";
  ...
  try {
    ...
    fs::path installPath = exePath / ".." / "share" / "vsr" / "scripts";
