...
class ImageCache
{
 public:
  ImageCache() = default;
  explicit ImageCache(vsr::scene::Scene *scene);

  // Copyable and moveable: an ImageCache is a value the caller owns, and
  // ImportContext holds one by value.
  VSR_DEFAULT_COPYABLE(ImageCache)
  VSR_DEFAULT_MOVEABLE(ImageCache)

  vsr::scene::Scene *scene() const;
...
vsr::scene::SamplerRef makeImageSampler(ImageCache &cache,
    const Image &image,
