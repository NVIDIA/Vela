...
void applyTransferFunction(
    Scene &scene, VolumeRef volume, const vsr::core::TransferFunction &tf)
{
  if (!volume) {
    logError(
  ...
  std::vector<vsr::math::float4> colormap;
    ...
    Scene &scene, const char *filepath, std::optional<std::string> propertyName)
{
  ...
  // '.vdb' is NanoVDB here, not OpenVDB: vsrVolumeToNanoVDB writes NanoVDB
