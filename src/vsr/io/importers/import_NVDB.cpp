...
#include "vsr/io/exporters/NanoVdbSidecar.hpp"
...
SpatialFieldRef import_NVDB(Scene &scene, const char *filepath)
{
  ...
  if (!hasNanoVdbMagic(filepath, magic)) {
    // '.vdb' names a NanoVDB grid here, not an OpenVDB one: vsrVolumeToNanoVDB
    ...
    if (nanovdb::toMagic(magic) == nanovdb::MagicType::OpenVDB) {
      logError(
          "[import_NVDB] '%s' is an OpenVDB file; VSR reads NanoVDB grids, "
          "which vsrVolumeToNanoVDB can convert one into",
          filepath);
    } else {
  ...
  try {
    ...
    if (!hasActiveVoxels) {
      ...
      if (VSR_NANOVDB_SKIP_INVALID_VOLUMES) {
        logStatus(
            "[import_NVDB] skipping due to VSR_NANOVDB_SKIP_INVALID_VOLUMES");
        scene.removeObject(field.data());
        return {};
      }
    }
    ...
    if (msg.find("compression codec was disabled"sv)
      ...
      logStatus(
          "[import_NVDB] '%s' is stored with a compressed codec but VSR was "
          "built without it. Reconfigure with -DVSR_NANOVDB_USE_ZIP=ON (and "
