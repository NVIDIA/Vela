...
#include "vsr/io/importers/detail/HDRImage.h"
#include "vsr/io/importers/detail/importer_common.hpp"
#include "vsr/io/importers/detail/pbrt/PbrtParser.hpp"
// stb
#include "stb_image.h"
// ply
#include "tinyply.h"
    ...
    std::map<MaterialCacheKey, MaterialRef> &matCache)
{
  ...
  if (type == "diffuse") {
    ...
    logWarning(
        ...
        type.c_str());
    mat = scene.createObject<Material>(tokens::material::matte);
    // VSR's matte default color is (1,0,0) — bright red. That makes every
  ...
  // VSR defaults alphaMode to "blend" (ANARI spec default is "opaque"); set
