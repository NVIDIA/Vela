    ...
    const std::string &fieldName)
{
  ...
  if (const char *env = std::getenv("VSR_SILO_IMPORT_RECTILINEAR_AS_UNIFORM")) {
  ...
  if (useUniformGrid) {
    ...
    logStatus(
        "[import_SILO] using uniform grid approximation (VSR_SILO_IMPORT_RECTILINEAR_AS_UNIFORM set)");
  } else {
    field = scene.createObject<SpatialField>(
