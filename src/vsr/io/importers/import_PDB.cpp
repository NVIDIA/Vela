...
struct Atom
{
  ...
  vsr::math::float3 position; // x, y, z coordinates
  ...
  vsr::math::float4 color; // Color of the atom
};
...
void parseAtomLine(const std::string &line, Atoms &atoms)
{
  ...
  atom.color = vsr::math::float4(1.f, 1.f, 1.f, 1.f);
  if (itColor != atomColorMap.end())
    atom.color = vsr::math::float4((*itColor).second.r / 255.f,
