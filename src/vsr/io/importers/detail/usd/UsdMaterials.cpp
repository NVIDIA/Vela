...
#include "vsr/io/importers/detail/usd/UsdMaterials.h"
// usd
...
// survive VSR's handoff: the document travels to the device as inline text,
    ...
    pxr::HdDataSourceMaterialNetworkInterface &networkInterface)
{
  ...
  for (const auto &nodePath : textures.hdTextureNodes) {
    ...
    for (const auto &inputName : inputNames->second) {
      ...
      // single image and neither VSR nor the device has anywhere to send the
...
// Write the generated document to `VSR_USD_MATERIALX_DUMP_DIR` when that is
// set, named after the material prim.
//
// The documents VSR emits are inline text handed straight to a device, so when
...
// point, and VSR is where the node names being complained about are minted.
void dumpDocument(const pxr::SdfPath &materialPath, const std::string &xml)
{
  const char *dir = std::getenv("VSR_USD_MATERIALX_DUMP_DIR");
...
// present in the document and importing them into the document VSR emits would
    ...
    const pxr::HdMaterialSchema &materialSchema)
{
  ...
  // The document names its own surface material node; VSR selects by that
  ...
  // against. VSR loads them here for the same reason it does for a preview
    ...
    const pxr::HdSceneIndexPrim &prim)
{
  ...
  auto setFloatIfPresent = [&](const char *usdName, const char *vsrName) {
    const auto value = walker.parameter(surfacePath, usdName);
    if (value.IsHolding<float>())
      material->setParameter(Token(vsrName), value.UncheckedGet<float>());
  };
