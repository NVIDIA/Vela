...
vsr::math::mat4 flattenedXformOf(
    const pxr::HdSceneIndexBaseRefPtr &sceneIndex, const pxr::SdfPath &primPath)
{
  ...
  return matrix ? toVsrMat4(matrix->GetTypedValue(0))
    ...
    InstancerRegistry &registry)
{
  ...
  const auto inverseRoot = vsr::math::inverse(rootXform);
  for (const pxr::SdfPath &path :
    ...
    const auto bake = content->internalTransformsAnimated
        ? vsr::math::IDENTITY_MAT4
        : vsr::math::mul(inverseRoot, flattenedXformOf(sceneIndex, path));
    for (auto &surface :
    ...
    LayerNodeRef parent)
{
  const auto inverseRoot =
      vsr::math::inverse(flattenedXformOf(sceneIndex, prototypeRoot));
...
std::vector<vsr::math::mat4> readInstanceTransforms(
    const pxr::HdSceneIndexPrim &prim, size_t instanceCount)
{
  ...
  std::vector<vsr::math::mat4> retval(instanceCount, vsr::math::IDENTITY_MAT4);
  ...
  for (size_t i = 0; i < retval.size(); ++i) {
    auto transform = vsr::math::IDENTITY_MAT4;

    if (scales.IsHolding<pxr::VtVec3fArray>()) {
      const auto &s = scales.UncheckedGet<pxr::VtVec3fArray>();
      if (i < s.size()) {
        transform = vsr::math::mul(
            vsr::math::scaling_matrix(float3(s[i][0], s[i][1], s[i][2])),
            transform);
      }
    }
    ...
    auto applyRotation = [&](float x, float y, float z, float w) {
      transform = vsr::math::mul(
          vsr::math::rotation_matrix(vsr::math::float4(x, y, z, w)), transform);
    };
    ...
    if (translations.IsHolding<pxr::VtVec3fArray>()) {
      ...
      if (i < t.size()) {
        transform = vsr::math::mul(
            vsr::math::translation_matrix(float3(t[i][0], t[i][1], t[i][2])),
            transform);
      }
    }
...
std::vector<vsr::math::mat4> InstancerPlacements::forPrototype(
    size_t prototypeIndex) const
{
  std::vector<vsr::math::mat4> retval;
