...
#include "vsr/io/usd/UsdDataSource.h"
...
/*
 * A material as VSR sees it, together with the primvar its own texture-reader
 ...
 * -- the `anari:` and `vsr:io:` attribute vocabularies, carrier metadata --
...
struct ImportContext
{
  ...
  Scene *scene{nullptr};
  vsr::animation::AnimationManager *animMgr{nullptr};
  ...
  vsr::animation::Animation &animation();
  ...
  // Prims the VSR dialect claimed, which every path that walks the resolved
...
vsr::math::mat4 toVsrMat4(const pxr::GfMatrix4d &m);
...
inline vsr::math::mat4 toVsrMat4(const pxr::GfMatrix4d &m)
{
  vsr::math::mat4 retval;
  for (int i = 0; i < 4; ++i)
    for (int j = 0; j < 4; ++j)
