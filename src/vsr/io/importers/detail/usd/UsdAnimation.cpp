...
float rotationDelta(const vsr::math::mat4 &a, const vsr::math::mat4 &b)
{
  float retval = 0.f;
  for (int axis = 0; axis < 3; ++axis) {
    const auto u = vsr::math::normalize(vsr::math::to_float3(a[axis]));
    const auto v = vsr::math::normalize(vsr::math::to_float3(b[axis]));
    retval = std::max(
        retval, std::acos(std::clamp(vsr::math::dot(u, v), -1.f, 1.f)));
  }
  return retval;
}
...
struct TransformSampler
{
  ...
  vsr::math::mat4 at(double time) const;
};

vsr::math::mat4 TransformSampler::at(double time) const
{
...
void densifyInterval(const TransformSampler &sampler,
    ...
    std::vector<vsr::math::mat4> &outFrames)
{
    ...
    ImportContext &ctx, const pxr::SdfPath &primPath, LayerNodeRef node)
{
  ...
  std::vector<vsr::math::mat4> frames;
