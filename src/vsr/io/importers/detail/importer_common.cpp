...
// These forward to vsr::io::images, which owns decoding, orientation, keying,
...
SamplerRef makeDefaultColorMapSampler(Scene &scene, const float2 &range)
{
  ...
  sampler->setParameter("inTransform", vsr::math::float2(range.x, range.y))
...
vsr::scene::ArrayRef makeArray1DFromVTK(
    vsr::scene::Scene &scene, vtkDataArray *array, const char *errorIdentifier)
{
    ...
    const std::vector<float> &timeBase)
{
  ...
  std::vector<vsr::core::math::float4> rotation(n);
  std::vector<vsr::core::math::float3> translation(n);
  std::vector<vsr::core::math::float3> scale(n);
