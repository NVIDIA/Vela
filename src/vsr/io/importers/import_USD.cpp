...
#if VSR_USE_USD
#include "vsr/io/importers/detail/usd/UsdAnimation.h"
#include "vsr/io/importers/detail/usd/UsdDialect.h"
#include "vsr/io/importers/detail/usd/UsdGeometry.h"
#include "vsr/io/importers/detail/usd/UsdImportContext.h"
#include "vsr/io/importers/detail/usd/UsdInstancing.h"
#include "vsr/io/importers/detail/usd/UsdLights.h"
#include "vsr/io/importers/detail/usd/UsdVolume.h"
...
struct Traversal
{
  ...
  void visit(const pxr::SdfPath &primPath,
      LayerNodeRef parent,
      bool hidden,
      const vsr::math::mat4 &parentXform);

  vsr::math::mat4 localTransformOf(
    ...
    const vsr::math::mat4 &parentXform)
{
  ...
  // accumulated ancestor transform instead, so VSR's own composition lands on
  // the same place USD does.
  const auto nodeXform = resetsXformStack
      ? vsr::math::mul(vsr::math::inverse(parentXform), localXform)
      : localXform;
  const auto accumulatedXform = vsr::math::mul(parentXform, nodeXform);
  ...
  if (prim.primType.IsEmpty()) {
    ...
    auto converted = convertGeometry(
        *ctx, sceneIndex, primPath, prim, vsr::math::IDENTITY_MAT4);
  ...
  if (scopeRoot == pxr::SdfPath::AbsoluteRootPath()) {
    for (const auto &childPath : sceneIndex->GetChildPrimPaths(scopeRoot)) {
      if (childPath.GetString() == NATIVE_INSTANCING_ROOT)
        continue;
      traversal.visit(childPath, root, false, vsr::math::IDENTITY_MAT4);
    }
  } else {
    traversal.visit(scopeRoot, root, false, vsr::math::IDENTITY_MAT4);
  }
...
UsdImportReport import_USD(Scene &scene,
    ...
    const UsdImportOptions &options)
{
  logError("[import_USD] USD not enabled in VSR build.");
  return {};
}
