    ...
    const std::optional<std::string> &propertyName = std::nullopt)
{
  ...
  auto *vertexPtr = vertexArray->mapAs<vsr::math::float3>();
  double p[3];
  for (vtkIdType i = 0; i < numPoints; ++i) {
    points->GetPoint(i, p);
    vertexPtr[i] = vsr::math::float3(static_cast<float>(p[0]),
  ...
  // Vertex data
  std::vector<vsr::scene::ArrayRef> vertexDataArrays;
  ...
  // Face data
  std::vector<vsr::scene::ArrayRef> faceDataArrays;
  ...
  auto mesh =
      scene.createObject<vsr::scene::Geometry>(tokens::geometry::triangle);
  ...
  if (colorArray) {
    auto colorRange = vsr::scene::computeScalarRange(*colorArray);
    ...
    const std::optional<std::string> &propertyName = std::nullopt)
{
  ...
  auto vertexArray = scene.createArray(ANARI_FLOAT32_VEC3, numPoints);
  auto *vertexData = vertexArray->mapAs<vsr::math::float3>();
  for (vtkIdType i = 0; i < numPoints; ++i) {
    double *pt = grid->GetPoint(i);
    vertexData[i] = vsr::math::float3(pt[0], pt[1], pt[2]);
  }
  vertexArray->unmap();
  ...
  auto makeField = [&](const std::string &name) -> SpatialFieldRef {
    auto f = scene.createObject<vsr::scene::SpatialField>(
    ...
    std::optional<std::string> propertyName)
{
  ...
  if (hasVolume) {
    ...
    if (field) {
      vsr::math::float2 valueRange = field->computeValueRange();
      auto [inst, volume] = scene.insertNewChildObjectNode<vsr::scene::Volume>(
      ...
      colorArr->setData(vsr::core::makeDefaultColorMap(256).data());
    ...
    std::optional<std::string> propertyName)
{
  (void)animMgr;
  (void)propertyName;
  logError("[import_VTU] VTK not enabled in VSR build.");
}
