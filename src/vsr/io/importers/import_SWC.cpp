    ...
    Scene &scene, const std::string &filename, LayerNodeRef location)
{
  ...
  for (const auto &p : points) {
    spherePositions.push_back(
        vsr::math::float3(p.second.x, p.second.y, p.second.z));
  ...
  for (const auto &p : points) {
    ...
    conePositions.push_back(vsr::math::float3(p1.x, p1.y, p1.z));
  ...
  vsr::math::float3 baseColor(0.5 + dis(gen), 0.5 + dis(gen), 0.5 + dis(gen));
