...
#include "vsr/io/usd/UsdResolvedGeometry.h"
#include "vsr/io/importers/detail/usd/UsdSubdivision.h"
...
// that decides which VSR attribute slot it lands in.
struct Primvar
{
  pxr::VtValue value;
...
// Which VSR parameter prefix an interpolation maps onto. Constant primvars
...
pxr::VtVec3fArray bakedPositions(
    const pxr::VtVec3fArray &source, const vsr::math::mat4 &bakeXform)
{
  if (bakeXform == vsr::math::IDENTITY_MAT4)
    return source;

  pxr::VtVec3fArray retval;
  retval.reserve(source.size());
  for (const auto &p : source) {
    const auto t = vsr::math::mul(bakeXform, float4(p[0], p[1], p[2], 1.f));
...
// USD authors widths; VSR geometry takes radii. Prims with no authored
    ...
    const pxr::VtVec3fArray &positions)
{
  ...
  if (!haveWidths) {
    ...
    for (const auto &p : positions) {
      ...
      lo = vsr::math::min(lo, v);
      hi = vsr::math::max(hi, v);
    }
    const float diagonal = positions.empty() ? 0.f : vsr::math::length(hi - lo);
...
// VSR hands ANARI run down the image. See
...
// is data VSR knows nothing about, and is resolved as authored.
void resolvePartPrimvar(ResolvedPart &part,
    const std::string &primvarName,
    const TriangulatedPrimvar &primvar,
    const std::vector<uint32_t> &triangles,
    const std::string &vsrName,
    bool isUv)
{
  const Token parameter((primvar.prefix + vsrName).c_str());
...
{
  ResolvedPart part;
  part.subtype = vsr::scene::tokens::geometry::triangle;
  part.name = name;
  ...
  auto bind = [&](const std::string &primvarName,
    ...
    if (found != attributes.end()) {
      resolvePartPrimvar(
          part, primvarName, found->second, triangles, vsrName, isUv);
    }
  };
    ...
    const GeometryResolveOptions &options)
{
  ...
  ResolvedPart part;
  part.subtype = vsr::scene::tokens::geometry::sphere;
    ...
    const GeometryResolveOptions &options)
{
  ...
  ResolvedPart part;
  part.subtype = vsr::scene::tokens::geometry::curve;
  part.name = primPath.GetString();
    ...
    const GeometryResolveOptions &options)
{
  ...
  const float3 origin = [&] {
    if (bakeXform == vsr::math::IDENTITY_MAT4)
      return float3(0.f);
    const auto t = vsr::math::mul(bakeXform, float4(0.f, 0.f, 0.f, 1.f));
  ...
  if (prim.primType == pxr::HdPrimTypeTokens->sphere) {
    ...
    if (bakeXform != vsr::math::IDENTITY_MAT4) {
      for (auto &e : endpoints) {
        const auto t = vsr::math::mul(bakeXform, float4(e.x, e.y, e.z, 1.f));
        e = float3(t.x, t.y, t.z);
      }
    }

    part.subtype = isCone ? vsr::scene::tokens::geometry::cone
    ...
    RefillCache &cache)
{
  ...
  for (const auto &attribute : part.attributes) {
    ...
    // A VSR Array's size is fixed at construction, so an element count that
