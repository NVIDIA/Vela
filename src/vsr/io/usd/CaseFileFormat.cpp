...
#include <vsr/io/importers/detail/ensight_io.hpp>
...
{
  vsr::io::ensight::CaseData caseData;
  if (!vsr::io::ensight::parseCase(resolvedPath, caseData))
    return false;
  ...
  std::string caseDir = vsr::io::pathOf(resolvedPath);
  ...
  if (geoPath.find('*') != std::string::npos) {
    auto expanded = vsr::io::ensight::expandPattern(
        caseData.geoPattern, caseData.startNumber, caseData.increment, 1);
    if (!expanded.empty())
      geoPath = caseDir + expanded[0];
  }

  std::vector<vsr::io::ensight::PartHeader> partHeaders;
  if (!vsr::io::ensight::readGeoFileHeader(geoPath, partHeaders))
    return false;
  ...
  for (const auto &ph : partHeaders) {
    std::string primName =
        vsr::io::ensight::sanitizePrimName(ph.description, ph.id);
