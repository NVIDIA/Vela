...
void writeMat4(core::DataNode &node, const vsr::math::mat4 &m)
{
...
vsr::math::mat4 readMat4(core::DataNode *node)
{
...
void UsdGeometryFileBinding::toDataNode(core::DataNode &node) const
{
  ...
  node["targetIndex"] = first ? first->index() : vsr::core::INVALID_INDEX;
...
UsdGeometryFileBinding *UsdGeometryFileBinding::addToAnimation(
    vsr::animation::Animation &anim, scene::Scene &scene, core::DataNode &node)
{
  ...
  if (auto *partsNode = node.child("parts")) {
    partsNode->foreach_child([&](core::DataNode &partNode) {
      const auto index =
          partNode["targetIndex"].getValueOr<size_t>(vsr::core::INVALID_INDEX);
