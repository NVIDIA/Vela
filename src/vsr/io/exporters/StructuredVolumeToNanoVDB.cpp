    ...
    bool enableDithering)
{
  ...
  vsr::core::logStatus("Exporting StructuredVolume to VDB file: %s",
      std::string(outputFilename).c_str());
  ...
  vsr::core::logStatus("[export_StructuredVolumeToNanoVDB] Data centering: %s",
      dataCentering.c_str());
  ...
  if (!writeSidecar(sidecar, sidecarPath, sidecarError)) {
    ...
    vsr::core::logStatus("[export_StructuredVolumeToNanoVDB] Wrote sidecar: %s",
        sidecarPath.string().c_str());
  }
}
