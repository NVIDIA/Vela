...
static std::filesystem::path vsrSamplerToFile(
...
static pxr::TfToken vsrSamplertoScalarOutputToken(const Sampler *sampler)
{
  assert(sampler);
    ...
    bool normalMap = false)
{
  ...
  auto textureFile = vsrSamplerToFile(stage, sampler);
...
static pxr::SdfPath vsrMaterialToPreviewSurfaceUSD(
...
static pxr::SdfPath vsrSurfaceToUSD(
    ...
    vsr::animation::AnimationManager *animMgr)
{
  ...
  vsr::core::logStatus("Exporting scene to USD file: %s", filename);

  pxr::UsdStageRefPtr stage = pxr::UsdStage::CreateNew(filename);
  if (!stage) {
    vsr::core::logError("Failed to create USD stage for file: %s", filename);
    return;
  }
    ...
    if (auto layer = l.second.ptr) {
      ...
      layer->traverse(layer->root(), [&](const LayerNode &node, int level) {
        ...
        if (node->isObject()) {
          ...
          case ANARI_SURFACE: {
            if (auto meshPath = vsrSurfaceToUSD(
                    stage, static_cast<const Surface *>(object));
                meshPath != pxr::SdfPath::EmptyPath()) {
              vsr::core::logInfo("Exporting surface %s (%p) to %s",
                  name.c_str(),
                  object,
                  meshPath.GetText());
  ...
  while (!textureWriteQueue.empty()) {
    for (auto &fut : textureWriteQueue) {
      if (fut.valid() && fut.wait_for(timeout) == std::future_status::ready) {
        vsr::core::logInfo(fut.get().c_str());
  ...
  vsr::core::logStatus("...done exporting USD scene to file: %s", filename);
}
...
void export_SceneToUSD(
    Scene &, const char *, int, vsr::animation::AnimationManager *)
{
  vsr::core::logError("[export_USD] USD not enabled in VSR build.");
}
