...
#include "vsr/io/archives/detail/AnimationRemap.hpp"
    ...
    const std::vector<std::string_view> &knownSchemas)
{
  ...
  if (metadataResult.found()) {
    ...
    result.message = "payload has no __vsr_metadata node; treating as legacy";
  }
...
static size_t remappedObjectIndex(const std::vector<ObjectIndexRemap> &remap,
    ...
    size_t index)
{
  if (index == vsr::core::INVALID_INDEX || type == ANARI_UNKNOWN)
    return index;
  const auto key = detail::makeKey(type, index);
  auto it = std::find_if(remap.begin(), remap.end(), [&](const auto &e) {
    return detail::sameKey(e.key, key);
  });
  return it == remap.end() ? vsr::core::INVALID_INDEX : it->newIndex;
}
...
{
  auto isExcluded = [&](const LayerNode &vsrNode) {
    return std::any_of(excludedNodes.begin(),
        excludedNodes.end(),
        [&](LayerNodeRef r) { return r && &(*r) == &vsrNode; });
  };
  ...
  for (auto l : scene.layers()) {
    ...
    l.second.ptr->traverse_const(
        l.second.ptr->root(), [&](const LayerNode &vsrNode, int) {
          if (isExcluded(vsrNode))
            return false; // pruned: not emitted, not counted
          remap.indices.emplace_back(vsrNode.index(), reloadIndex++);
          return true;
        });
    ...
    size_t index)
{
  for (const auto &r : remaps) {
    ...
    return vsr::core::INVALID_INDEX; // node was pruned
  }
    ...
    const LegacySceneSerializationOptions &options)
{
  ...
  vsr::core::logStatus("    ...serializing %zu layers", scene.numberOfLayers());
