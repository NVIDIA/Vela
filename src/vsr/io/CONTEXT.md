...
VSR I/O moves data between VSR and external representations and persists
native VSR state. Its language applies across VSR and is independent of the
...
**Archive**:
A native serialized representation of some VSR state. An Archive is independent
of its carrier: it may be embedded in another representation, transmitted
directly, or saved as a standalone `.vsr` file. Archive formats may carry
schema metadata when their validation or evolution requires it.

**Scene Archive**:
An Archive containing the current state of one entire VSR scene but no
...
**Object Archive**:
An Archive containing one primary VSR scene object and the object dependencies
...
**Animation Archive**:
An Archive containing the state of one VSR animation. Its scene-object and
...
**Save**:
Persist a native VSR Archive or Application Dump to a file. The operation may
serialize an intermediate representation, but its boundary is a file.

**Load**:
Reconstruct native VSR state from an Archive or Application Dump stored in a
file. The operation may deserialize an intermediate representation, but its
boundary originates at a file.

**Serialize**:
Convert runtime VSR state into its native in-memory representation without
implying file I/O.

**Deserialize**:
Reconstruct runtime VSR state from its native in-memory representation without
implying file I/O.
...
**Import**:
Convert a non-native representation into VSR state.
_Avoid_: Load, deserialize

**Export**:
Convert VSR state into a non-native representation.
...
**Time Code**:
The Stage's own clock, distinct from VSR's normalized animation time. An Import
...
qualified: unqualified **Layer** means a VSR Layer and never this.
_Avoid_: Layer (unqualified), sublayer

**Prototype**:
Content authored once and shared by every placement of it. An imported
Prototype is one set of VSR scene objects referenced from many layer nodes,
...
primvar already expanded and gathered — with no VSR scene objects involved.
...
renderable VSR content would have been.

**Import Report**:
A structured record of one Import: which prims became VSR scene objects and,
