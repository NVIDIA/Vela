    ...
    ArchiveValidationResult &result)
{
  ...
  size_t declaredRootIndex = vsr::core::INVALID_INDEX;
    ...
    Scene &scene, core::DataNode &root, ArchiveValidationResult *validation)
{
  ...
  if (validation)
    *validation = result;
  if (!result.accepted()) {
    vsr::core::logError("[deserialize_ObjectArchive] validation failed: %s",
  ...
  if (!instantiateObjectDB(
    ...
    vsr::core::logError("[deserialize_ObjectArchive] %s", errorMessage.c_str());
    return nullptr;
  }
...
bool serialize_ObjectArchive(const Object &obj, core::DataNode &root)
{
  ...
  if (!buildClosure(*scene,
          ...
          entries,
          errorMessage)) {
    vsr::core::logError("[serialize_ObjectArchive] %s", errorMessage.c_str());
    return false;
  }
...
{
  if (!filename) {
    vsr::core::logError("[load_ObjectArchive] filename is null");
    return nullptr;
  }
