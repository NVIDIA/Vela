  ...
  if (data && count > 0) {
    if (anari::isObject(type)) {
      ...
      for (size_t i = 0; i < count; i++) {
        ...
        indices[i] = object ? object->index() : VSR_INVALID_INDEX;
        if (object)
    ...
    const ObjectParameterBinding &binding, const TimeSample &sample)
{
  ...
  if (binding.interpolation() == InterpolationRule::LINEAR) {
    ...
    case ANARI_FLOAT32: {
      ...
      float v = vsr::math::lerp(arr[sample.lo], arr[sample.hi], sample.alpha);
      return core::Any(v);
    }
    case ANARI_FLOAT32_VEC2: {
      auto *arr = reinterpret_cast<const math::float2 *>(data);
      auto v = vsr::math::lerp(arr[sample.lo], arr[sample.hi], sample.alpha);
      return core::Any(v);
    }
...
void ObjectParameterBinding::toDataNode(core::DataNode &node) const
{
  ...
  node["targetIndex"] = m_target ? m_target->index() : VSR_INVALID_INDEX;
...
void ObjectParameterBinding::fromDataNode(core::DataNode &node)
{
  ...
  if (targetType != ANARI_UNKNOWN && targetIndex != VSR_INVALID_INDEX)
  ...
  if (anari::isObject(m_type) && dataPtr && dataCount > 0) {
    ...
    for (size_t i = 0; i < dataCount; i++) {
      if (indices[i] != VSR_INVALID_INDEX)
