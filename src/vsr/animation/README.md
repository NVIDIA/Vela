## Animation Library (`vsr_animation`)

`vsr_animation` provides time-based parameter and transform interpolation for
VSR scenes.
...
- **`core::AnyArray`** — owning, type-aware keyframe value buffer (from
  `vsr_core`) backed by `std::vector<uint8_t>`. Provides typed accessors
