...
vsr::math::float3 interpolateColor(
    const std::vector<ColorPoint> &controlPoints, float x)
{
  if (controlPoints.empty())
    return vsr::math::float3(0.f);

  auto first = controlPoints.front();
  if (x <= first.x)
    return vsr::math::float3(first.y, first.z, first.w);
  ...
  for (uint32_t i = 1; i < controlPoints.size(); i++) {
    ...
    if (x <= current.x) {
      ...
      return (1.0f - t) * vsr::math::float3(previous.y, previous.z, previous.w)
          + t * vsr::math::float3(current.y, current.z, current.w);
    }
  }

  auto last = controlPoints.back();
  return vsr::math::float3(last.y, last.z, last.w);
}
