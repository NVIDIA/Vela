...
inline Any::Any(T value) : Any()
{
  ...
  static_assert(type != ANARI_UNKNOWN, "unknown type used initialize vsr::Any");
...
{
  if (!is<T>())
    throw std::runtime_error("get() called with invalid type on vsr::Any");
  return getAs<T>(type());
}
...
inline T Any::getAs(anari::DataType expectedType) const
{
  ...
  if (!valid())
    throw std::runtime_error("Any::getAs<>() called on empty vsr::Any");
