...
#include "vsr/ui/imgui/vsr_font.h"
...
vsr::app::Context *Application::appContext()
{
  return &m_ctx;
}
    ...
    std::string &filenameOut, FileDialogMode mode)
{
  auto fileDialogCb =
      [](void *userdata, const char *const *filelist, int filter) {
        ...
        if (!filelist) {
          vsr::core::logError("SDL DIALOG ERROR: %s\n", SDL_GetError());
          return;
        }
...
WindowArray Application::setupWindows()
{
  ...
  auto *font = io.Fonts->AddFontFromMemoryCompressedTTF(
      vsr_font_compressed_data, vsr_font_compressed_size, 20.f);
  ...
  vsr::ui::setupArrayPreview(sdlRenderer());
...
void Application::uiFrameStart()
{
  ...
  m_ctx.vsr.animationMgr.tick(ImGui::GetIO().DeltaTime);
  ...
  if (!io.WantTextInput && ImGui::IsKeyPressed(ImGuiKey_Space))
    m_ctx.vsr.animationMgr.togglePlay();
...
void Application::uiMainMenuBar_File()
{
  if (ImGui::BeginMenu("File")) {
    ...
    tooltipForPreviousItem("Load session from a .vsr file");
    ...
    tooltipForPreviousItem("Save session to a .vsr file");
    ...
    tooltipForPreviousItem("Save sesson to 'state.vsr' in the local directory");
    ...
    if (ImGui::MenuItem("Export as USD...")) {
      io::export_SceneToUSD(m_ctx.vsr.scene,
          "scene.usda",
  ...
  if (ImGui::BeginMenu("Edit")) {
    ...
    if (ImGui::BeginMenu("Scene")) {
      if (ImGui::MenuItem("Cleanup Unused Objects"))
        m_ctx.vsr.scene.removeUnusedObjects(false);

      if (ImGui::MenuItem("Cleanup Unused Objects + Renderers"))
        m_ctx.vsr.scene.removeUnusedObjects(true);

      if (ImGui::MenuItem("Defragment Scene Storage"))
        m_ctx.vsr.scene.defragmentObjectStorage();
...
void Application::uiMainMenuBar_Tools()
{
  if (ImGui::BeginMenu("Tools")) {
    ...
    if (ImGui::BeginMenu("VSR Device")) {
      ...
      ImGui::Separator();
      ImGui::BeginDisabled(!vsrDeviceIsSetup());
      if (ImGui::MenuItem("Sync"))
        syncVsrScene();
      ImGui::EndDisabled();
      ImGui::EndMenu();
...
void Application::saveApplicationState(const char *_filename)
{
  ...
  auto doSave = [&, filename = f_str]() {
    vsr::core::logStatus("clearing old settings tree...");
    ...
    vsr::core::logStatus("...state saved to '%s'", filename.c_str());
...
vsr::core::DataTreeMetadata Application::applicationStateMetadata() const
{
  return {vsr::core::DATA_TREE_METADATA_ENVELOPE_VERSION,
      "application-state",
      "vsr.ui.imgui.application-state",
      1};
}

bool Application::validateApplicationStateMetadata(
    const vsr::core::DataTreeMetadataReadResult &,
    const vsr::core::DataNode &,
    const char *) const
{
  return true;
}
...
void Application::loadApplicationState(const char *filename)
{
  ...
  if (!m_settings.load(filename)) {
    vsr::core::logError("failed to load state from '%s'", filename);
    return;
  }
  ...
  if (!validateApplicationStateMetadata(
          vsr::core::readDataTreeMetadata(root), root, filename)) {
    root.reset();
    return;
  }

  if (!vsr::app::deserialize_ApplicationDump(ctx, root)) {
    root.reset();
    return;
  }
...
void Application::setupUsdDevice()
{
  ...
  if (d == nullptr) {
    ...
    if (!d) {
      vsr::core::logWarning("USD device failed to load");
      return;
    }
    anari::retain(d, d);
  ...
  m_usdDevice.renderIndex =
      m_ctx.anari.acquireRenderIndex(m_ctx.vsr.scene, "usd", d);
...
{
  vsr::core::logStatus("synchronizing USD ANARI device scene...");
  if (!usdDeviceIsSetup()) {
    vsr::core::logWarning("USD device not setup -- cannot sync scene");
    return;
  }
  ...
  timer.end();
  vsr::core::logStatus("...sync complete (%.2f ms)", timer.milliseconds());
}
...
void Application::teardownUsdDevice()
{
  if (!usdDeviceIsSetup())
    return;
  vsr::core::logStatus("tearing down USD device...");
  auto d = m_usdDevice.device;
  m_ctx.anari.releaseRenderIndex(m_ctx.vsr.scene, d);
...
void Application::setupVsrDevice()
{
  if (vsrDeviceIsSetup())
    return;

  auto d = m_vsrDevice.device;

  if (d == nullptr) {
    d = m_ctx.anari.loadDevice("vsr");
    if (!d) {
      vsr::core::logWarning("VSR device failed to load");
      return;
    }
    anari::retain(d, d);
    m_vsrDevice.device = d;
  }

  m_vsrDevice.renderIndex =
      m_ctx.anari.acquireRenderIndex(m_ctx.vsr.scene, "vsr", d);
  m_vsrDevice.frame = anari::newObject<anari::Frame>(d);
  anari::setParameter(
      d, m_vsrDevice.frame, "world", m_vsrDevice.renderIndex->world());

  syncVsrScene();
}

bool Application::vsrDeviceIsSetup() const
{
  return m_vsrDevice.device != nullptr && m_vsrDevice.renderIndex != nullptr;
}

void Application::syncVsrScene()
{
  vsr::core::logStatus("synchronizing VSR ANARI device scene...");
  if (!vsrDeviceIsSetup()) {
    vsr::core::logWarning("VSR device not setup -- cannot sync scene");
    return;
  }
  vsr::core::Timer timer;
  timer.start();
  anari::render(m_vsrDevice.device, m_vsrDevice.frame);
  anari::wait(m_vsrDevice.device, m_vsrDevice.frame);
  timer.end();
  vsr::core::logStatus("...sync complete (%.2f ms)", timer.milliseconds());
}

void Application::teardownVsrDevice()
{
  if (!vsrDeviceIsSetup())
    return;
  vsr::core::logStatus("tearing down VSR device...");
  auto d = m_vsrDevice.device;
  m_ctx.anari.releaseRenderIndex(m_ctx.vsr.scene, d);
  anari::release(d, m_vsrDevice.frame);
  anari::release(d, d);
  m_vsrDevice.device = nullptr;
  m_vsrDevice.renderIndex = nullptr;
}
