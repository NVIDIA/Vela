...
#include "vsr/ui/imgui/ArrayPreview.h"
// vsr_core
...
{
  const auto &stringList = ((vsr::scene::Parameter *)p)->stringValues();
  *out_text = stringList[index].c_str();
  return true;
}

static void buildUI_array_info_tooltip_text(
    const vsr::scene::Scene &scene, size_t idx)
{
  const auto &a = *scene.getObject<vsr::scene::Array>(idx);
  if (vsr::ui::buildUI_array_preview(a))
    ImGui::Separator();
    ...
    const vsr::scene::Scene &scene, size_t idx)
{
  ...
  if (auto *p = s->parameter("image"); p) {
    ...
    if (aIdx != VSR_INVALID_INDEX) {
      if (auto arr = scene.getObject<vsr::scene::Array>(aIdx); arr)
        if (vsr::ui::buildUI_array_preview(*arr))
          ImGui::Separator();
    }
  }
  ImGui::Text(" idx: [%zu]", idx);
...
static bool buildUI_object_info_tooltip_text(
    const vsr::scene::Scene &scene, anari::DataType type, size_t idx)
{
    ...
    if (ImGui::BeginMenu("set type")) {
      if (ImGui::BeginMenu("uniform")) {
        if (ImGui::MenuItem("direction")) {
          ...
          p->setValue(vsr::math::float2(0.f));
        }

        if (ImGui::BeginMenu("color")) {
          if (ImGui::MenuItem("rgb") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::COLOR);
            p->setValue(vsr::math::float3(1));
          }
          if (ImGui::MenuItem("rgba") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::COLOR);
            p->setValue(vsr::math::float4(1));
          }
          ImGui::EndMenu(); // "color"
        }
        ...
        if (ImGui::BeginMenu("transform")) {
          if (ImGui::MenuItem("identity") && p) {
            ...
            p->setValue(vsr::math::scaling_matrix(vsr::math::float3(1.f)));
          }
          ImGui::Separator();
          if (ImGui::MenuItem("value range") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::VALUE_RANGE_TRANSFORM);
            p->setValue(vsr::math::float2(0.f, 1.f));
          }
        ...
        if (ImGui::BeginMenu("float")) {
          ...
          if (ImGui::MenuItem("float2") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::NONE);
            p->setValue(vsr::math::float2(1.f));
          }
          if (ImGui::MenuItem("float3") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::NONE);
            p->setValue(vsr::math::float3(1.f));
          }
          if (ImGui::MenuItem("float4") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::NONE);
            p->setValue(vsr::math::float4(1.f));
          }
          ImGui::EndMenu(); // "float"
        }
        ...
        if (ImGui::BeginMenu("int")) {
          ...
          if (ImGui::MenuItem("int2") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::NONE);
            p->setValue(vsr::math::int2(1));
          }
          if (ImGui::MenuItem("int3") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::NONE);
            p->setValue(vsr::math::int3(1));
          }
          if (ImGui::MenuItem("int4") && p) {
            p->setUsage(vsr::scene::ParameterUsageHint::NONE);
            p->setValue(vsr::math::int4(1));
          }
          ImGui::EndMenu(); // "int"
        }
...
#define OBJECT_UI_MENU_ITEM(text, name)                                        \
  if (ImGui::MenuItem(text)) {                                                 \
    ...
    auto colormap = vsr::core::resampleArray(vsr::core::colormap::name, 128);  \
    a->setData(colormap);                                                      \
...
#define OBJECT_UI_MENU_ITEM(text, name)                                        \
  if (ImGui::MenuItem(text)) {                                                 \
    const auto &colors = vsr::core::palette::name;                             \
...
#undef OBJECT_UI_MENU_ITEM
          ...
          if (ImGui::BeginMenu("material")) {
            vsr::scene::MaterialRef m;
            if (ImGui::MenuItem("matte")) {
              m = scene.createObject<vsr::scene::Material>(
            ...
            if (ImGui::BeginMenu("MDL")) {
              if (ImGui::MenuItem("mdl")) {
                m = scene.createObject<vsr::scene::Material>(
                    vsr::scene::tokens::material::mdl);
              }
              if (ImGui::MenuItem("physicallyBasedMDL")) {
                m = scene.createObject<vsr::scene::Material>(
                    vsr::scene::tokens::material::physicallyBasedMDL);
              }
              ImGui::EndMenu(); // "MDL"
            }
            ...
            if (ImGui::BeginMenu("MaterialX")) {
              ...
              if (ImGui::MenuItem("StandardSurface")) {
                m = scene.createObject<vsr::scene::Material>(
                    vsr::scene::tokens::material::materialx);
                vsr::scene::applyMaterialXStandardSurfacePreset(*m);
              }
    ...
    int level)
{
  ...
  if (anari::isArray(o.type())) {
    const auto &a = *(vsr::scene::Array *)&o;
    const auto t = a.type();
    ...
    if (t == ANARI_ARRAY3D)
      ...
      ImGui::Text(" size: %zu", a.dim(0));
    ImGui::Text(" type: %s", anari::toString(a.elementType()));
    vsr::ui::buildUI_array_preview(a);
  ...
  if (o.type() == ANARI_SAMPLER) {
    if (auto *p = o.parameter("image"); p) {
      ...
      if (idx != VSR_INVALID_INDEX) {
        if (auto arr = scene.getObject<vsr::scene::Array>(idx); arr)
          vsr::ui::buildUI_array_preview(*arr);
      }
    }
  }
  ...
  ImGui::Text("use counts: [ %zu | %zu | %zu | %zu | %zu ]",
      o.useCount(vsr::scene::Object::UseKind::APP),
      o.useCount(vsr::scene::Object::UseKind::PARAMETER),
      o.useCount(vsr::scene::Object::UseKind::LAYER),
      o.useCount(vsr::scene::Object::UseKind::ANIM),
      o.useCount(vsr::scene::Object::UseKind::INTERNAL));
    ...
    bool useTable)
{
    ...
    if (bounded) {
      if (pMin && pMax) {
        update |= ImGui::SliderFloat2(name,
            (float *)value,
            pMin.get<vsr::math::float2>().x,
            pMax.get<vsr::math::float2>().x);
      } else {
        float min = pMin ? pMin.get<vsr::math::float2>().x
                         : std::numeric_limits<float>::lowest();
        float max = pMax ? pMax.get<vsr::math::float2>().x
      ...
      if (bounded) {
        if (pMin && pMax) {
          update |= ImGui::SliderFloat2("lower",
              (float *)value,
              pMin.get<vsr::math::box2>().lower.x,
              pMax.get<vsr::math::box2>().lower.x);
          update |= ImGui::SliderFloat2("upper",
              (float *)value + 2,
              pMin.get<vsr::math::box2>().upper.x,
              pMax.get<vsr::math::box2>().upper.x);
        } else {
          float minLower = pMin ? pMin.get<vsr::math::box2>().lower.x
                                : std::numeric_limits<float>::lowest();
          float maxLower = pMax ? pMax.get<vsr::math::box2>().lower.x
                                : std::numeric_limits<float>::max();
          update |= ImGui::DragFloat2(
              "lower", (float *)value, 1.f, minLower, maxLower);
          float minUpper = pMin ? pMin.get<vsr::math::box2>().upper.x
                                : std::numeric_limits<float>::lowest();
          float maxUpper = pMax ? pMax.get<vsr::math::box2>().upper.x
                                : std::numeric_limits<float>::max();
          update |= ImGui::DragFloat2(
      ...
      if (bounded) {
        if (pMin && pMax) {
          update |= ImGui::SliderFloat3("lower",
              (float *)value,
              pMin.get<vsr::math::box3>().lower.x,
              pMax.get<vsr::math::box3>().lower.x);
          update |= ImGui::SliderFloat3("upper",
              (float *)value + 3,
              pMin.get<vsr::math::box3>().upper.x,
              pMax.get<vsr::math::box3>().upper.x);
        } else {
          float minLower = pMin ? pMin.get<vsr::math::box3>().lower.x
                                : std::numeric_limits<float>::lowest();
          float maxLower = pMax ? pMax.get<vsr::math::box3>().lower.x
                                : std::numeric_limits<float>::max();
          update |= ImGui::DragFloat3(
              "lower", (float *)value, 1.f, minLower, maxLower);
          float minUpper = pMin ? pMin.get<vsr::math::box3>().upper.x
                                : std::numeric_limits<float>::lowest();
          float maxUpper = pMax ? pMax.get<vsr::math::box3>().upper.x
                                : std::numeric_limits<float>::max();
          update |= ImGui::DragFloat3(
  ...
  case ANARI_FLOAT32_VEC3:
    if (usage & vsr::scene::ParameterUsageHint::COLOR)
  ...
  case ANARI_FLOAT32_MAT4: {
    ...
    if (ImGui::CollapsingHeader(name)) {
      ...
      if (ImGui::Button("identity")) {
        if (N == 3) {
          auto I = vsr::math::IDENTITY_MAT3;
          std::memcpy(m, &I, sizeof(I));
        } else {
          auto I = vsr::math::IDENTITY_MAT4;
          std::memcpy(m, &I, sizeof(I));
        }
        update = true;
      }
  ...
  default:
    if (const auto idx = pVal.getAsObjectIndex(); idx != VSR_INVALID_INDEX) {
      if (useTable)
...
{
  size_t retval = VSR_INVALID_INDEX;
