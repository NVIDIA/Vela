...
void TransferFunctionEditor::buildUI_valueRange()
{
  ...
  if (ImGui::Button("reset##valueRange") && m_volume) {
    auto *field =
        m_volume->parameterValueAsObject<vsr::scene::SpatialField>("value");
    if (field) {
...
std::vector<vsr::math::float4>
TransferFunctionEditor::getSampledColorsAndOpacities(int numSamples)
{
  std::vector<vsr::math::float4> sampledColorsAndOpacities;
  ...
  for (int i = 0; i < numSamples; i++) {
    ...
    vsr::math::float3 color(0.f);
    if (interpolateColor)
      color = vsr::core::detail::interpolateColor(*m_tfnColorPoints, i * dx);
    else {
      const auto co = (*m_tfnColorPoints)[i];
      color = vsr::math::float3(co.x, co.y, co.z);
    }
    const auto opacity =
        vsr::core::detail::interpolateOpacity(m_tfnOpacityPoints, i * dx);
    sampledColorsAndOpacities.push_back(vsr::math::float4(color, opacity));
  }
...
void TransferFunctionEditor::setObjectPtrsFromSelectedObject()
{
  ...
  for (const auto &node : selectedNodes) {
    ...
    if (obj && obj->type() == ANARI_VOLUME) {
      allVolumes.push_back((vsr::scene::Volume *)obj);
    }
  }
...
{
  std::vector<vsr::core::ColorPoint> colors;

  // Incoming color map
  m_tfnsNames.push_back("{from volume}");
  m_tfnsColorPoints.push_back(colors);

  auto addColorMap = [&](const std::vector<vsr::math::float3> &colormap,
  ...
  addColorMap(vsr::core::colormap::jet, "Jet");
  addColorMap(vsr::core::colormap::cool_to_warm, "Cool to Warm");
  addColorMap(vsr::core::colormap::viridis, "Viridis");
  addColorMap(vsr::core::colormap::black_body, "Black-Body Radiation");
  addColorMap(vsr::core::colormap::inferno, "Inferno");
  addColorMap(vsr::core::colormap::ice_fire, "Ice Fire");
    ...
    const std::string &filepath, const std::string &name)
{
  ...
  core::TransferFunction tfn = vsr::io::importTransferFunction(filepath);
  ...
  std::vector<vsr::core::OpacityPoint> smartOpacityPoints;
...
void TransferFunctionEditor::updateColormaps()
{
  if (!m_colorMapArray) {
    ...
    auto promoteVolume = [&](vsr::scene::Volume *vol) {
      auto *s = vol->scene();
      if (!s)
        return;
...
void TransferFunctionEditor::updateTfnPaletteTexture()
{
  ...
  std::vector<vsr::math::float4> palette = m_currentMap == 0
      ? *m_tfnColorPoints
      : getSampledColorsAndOpacities(width);
  std::vector<vsr::math::float3> rgb(width, vsr::math::float3(1.f, 0.f, 0.f));
  std::transform(palette.begin(), palette.end(), rgb.begin(), [&](auto &c) {
    return vsr::math::float3(c.x, c.y, c.z);
  });

  SDL_UpdateTexture(m_tfnPaletteTexture,
      nullptr,
      rgb.data(),
      width * sizeof(vsr::math::float3));
}
...
void TransferFunctionEditor::saveColormapTo1dt(const std::string &filepath)
{
  ...
  if (colorsAndOpacities.empty()) {
    vsr::core::logError("[tfn_editor] No colormap data to save");
    return;
  }
...
void TransferFunctionEditor::saveColormapToParaview(const std::string &filepath)
{
  ...
  if (m_tfnOpacityPoints.empty()) {
    vsr::core::logError("[tfn_editor] No opacity control points to save");
    return;
  }
    ...
    if (m_currentMap == 0) {
      ...
      if (!m_tfnColorPoints->empty()) {
        ...
        if (position <= m_tfnColorPoints->front().x) {
          ...
          color = vsr::math::float3(cp.y, cp.z, cp.w);
        } else if (position >= m_tfnColorPoints->back().x) {
          auto &cp = m_tfnColorPoints->back();
          color = vsr::math::float3(cp.y, cp.z, cp.w);
        } else {
          color =
              vsr::core::detail::interpolateColor(*m_tfnColorPoints, position);
        }
      }
    } else {
      // Interpolated color mapping
      color = vsr::core::detail::interpolateColor(*m_tfnColorPoints, position);
    }
