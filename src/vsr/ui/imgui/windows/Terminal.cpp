...
#include "vsr/ui/imgui/ExtensionManager.h"
#endif
...
Terminal::Terminal(Application *app) : Window(app, "Terminal")
{
  ...
  addOutput("VSR Lua Terminal\n");
  addOutput("Type 'help' for usage hints.\n");
  addOutput("'scene' is bound to the current VSR scene.\n\n");
#else
  addOutput("Lua scripting is not enabled.\n");
  addOutput("Build with -DVSR_USE_LUA=ON to enable.\n");
#endif
}
