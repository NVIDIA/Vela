...
#include "vsr/ui/imgui/modals/ExportNanoVDBFileDialog.h"
#include "vsr/ui/imgui/modals/ImportFileDialog.h"
...
{
  const auto &layers = *(const vsr::scene::LayerMap *)l;
...
{
  if (!appContext()->vsr.sceneLoadComplete) {
...
std::vector<vsr::scene::LayerNodeRef> LayerTree::computeSelectionRange(
    vsr::scene::Layer &layer,
    const vsr::scene::LayerNodeRef &anchor,
    const vsr::scene::LayerNodeRef &target)
{
  std::vector<vsr::scene::LayerNodeRef> range;
  ...
  if (!foundFirst || !foundSecond) {
    return std::vector<vsr::scene::LayerNodeRef>();
  }
...
void LayerTree::buildUI_tree()
{
  ...
  if (ImGui::BeginTable("objects", 1, flags)) {
    ...
    vsr::scene::LayerNodeRef dragAndDropTarget = {};
    std::vector<vsr::scene::LayerNodeRef> droppedNodes;
    ...
    auto onNodeEntryBuildUI = [&](auto &node, int level) {
      ...
      vsr::scene::Object *obj = node->getObject();
      ...
      if (ImGui::IsItemClicked() && m_menuNode == VSR_INVALID_INDEX) {
      ...
      if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
        ...
        ImGui::SetDragDropPayload("LAYER_TREE_NODE",
            data(draggedNodes),
            sizeof(vsr::scene::LayerNodeRef) * size(draggedNodes));
      ...
      if (ImGui::BeginDragDropTarget()) {
        ...
        if (const ImGuiPayload *payload = ImGui::GetDragDropPayload()) {
          if (payload->IsDataType("LAYER_TREE_NODE")) {
            ...
            auto *nodes = (vsr::scene::LayerNodeRef *)payload->Data;
            size_t count = payload->DataSize / sizeof(vsr::scene::LayerNodeRef);
    ...
    if (dragAndDropTarget.valid() && !droppedNodes.empty()) {
      ...
      appContext()->vsr.scene.signalLayerStructureChanged(&layer);
    }
  }
}
...
void LayerTree::buildUI_handleSelection()
{
  ...
  if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_X, false)) {
    ...
    if (!parentOnlyNodes.empty()) {
      ...
      appContext()->vsr.stashedSelection.shouldDeleteAfterPaste = true;
    }
  }
  ...
  if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_C, false)) {
    ...
    if (!parentOnlyNodes.empty()) {
      appContext()->vsr.stashedSelection.nodes = parentOnlyNodes;
      appContext()->vsr.stashedSelection.shouldDeleteAfterPaste = false;
    }
  }
...
#undef OBJECT_UI_MENU_ITEM
    ...
    if (canExport) {
      ImGui::Separator();
      if (ImGui::MenuItem("save VSR Object Archive")) {
        auto objectType = menuObject->type() == ANARI_VOLUME
            ? VSRObjectFileType::Volume
            : VSRObjectFileType::Surface;
    ...
    if (nodeSelected) {
      if (!canExport)
        ImGui::Separator();
      if (ImGui::MenuItem("save VSR Layer Subtree Archive"))
...
void LayerTree::buildUI_newLayerSceneMenu()
{
  if (ImGui::BeginPopup("LayerTree_contextMenu_newLayer")) {
    ...
    if ((ImGui::Button("ok") || ImGui::IsKeyDown(ImGuiKey_Enter))
      ...
      auto &scene = appContext()->vsr.scene;
      vsr::core::Token layerName = s_newLayerName.c_str();
...
std::vector<vsr::scene::LayerNodeRef> LayerTree::copyNodesTo(
    ...
    bool cutOperation)
{
  ...
  std::vector<vsr::scene::LayerNodeRef> validNodes;
  ...
  std::vector<vsr::scene::LayerNodeRef> newNodes;
...
bool LayerTree::isValidDropTarget(vsr::scene::Layer &layer,
    vsr::scene::LayerNodeRef targetParent,
