...
void BaseViewport::buildUI()
{
  ...
  const vsr::math::int2 requested(int(viewportSize.x), int(viewportSize.y));
...
void BaseViewport::setManipulator(vsr::rendering::Manipulator *m)
{
...
void BaseViewport::viewport_reshape(vsr::math::int2 newWindowSize)
{
  if (newWindowSize.x <= 0 || newWindowSize.y <= 0)
    return;

  m_viewport.size = newWindowSize;
  m_viewport.renderSize = vsr::math::int2(
      vsr::math::float2(m_viewport.size) * m_viewport.resolutionScale);
...
const vsr::rendering::ImagePipeline &BaseViewport::imagePipeline() const
{
  return m_pipeline;
}
...
void BaseViewport::camera_update(bool force)
{
  ...
  if (!m_camera.current)
    camera_setCurrent(appContext()->vsr.scene.defaultCamera());
...
void BaseViewport::camera_setCurrent(vsr::scene::CameraAppRef c)
{
  m_camera.current = c;
}
...
void BaseViewport::ui_handleInput()
{
  ...
  if (!anyMovement) {
    ...
    m_input.previousMouse = vsr::math::float2(-1);
  ...
  if (m_input.manipulating) {
    ...
    if (anyMovement && m_input.previousMouse != vsr::math::float2(-1)) {
      const vsr::math::float2 prev = m_input.previousMouse;

      const vsr::math::float2 mouseFrom =
          prev * 2.f / vsr::math::float2(m_viewport.size);
      const vsr::math::float2 mouseTo =
          mouse * 2.f / vsr::math::float2(m_viewport.size);
...
void BaseViewport::ui_gizmo()
{
  ...
  auto computeWorldTransform = [](vsr::scene::LayerNodeRef node) -> math::mat4 {
  ...
  if (ImGuizmo::Manipulate(&view[0].x,
    ...
    appContext()->vsr.scene.signalLayerTransformChanged(
    ...
    ImGui::Text("Current Camera:");
    {
      ...
      if (ImGui::BeginMenu("Select Camera")) {
        if (ImGui::BeginMenu("New")) {
          vsr::scene::CameraRef newCam;
          if (ImGui::MenuItem("Perspective")) {
            newCam = scene.createObject<vsr::scene::Camera>(
          ...
          if (ImGui::MenuItem("Omnidirectional")) {
            newCam = scene.createObject<vsr::scene::Camera>(
                vsr::scene::tokens::camera::omnidirectional);
          }
        ...
        if (auto i = vsr::ui::buildUI_objects_menulist(scene, t);
            i != VSR_INVALID_INDEX) {
          camera_setCurrent(scene.getObject<vsr::scene::Camera>(i));
...
void BaseViewport::applyViewMatrixToArcball(const float *viewMat)
{
  ...
  const vsr::math::float3 forward{-viewMat[2], -viewMat[6], -viewMat[10]};

  // d = direction from 'at' toward new 'eye' = -forward
  const vsr::math::float3 d = -forward;
  ...
  case vsr::rendering::UpAxis::POS_Y: {
    // azelToDirection = -normalize({sin(az)*cos(el), sin(el), cos(az)*cos(el)})
    const vsr::math::float3 D = -d;
    el_rad = std::asin(D.y);
    az_rad = std::atan2(D.x, D.z);
    break;
  }
  case vsr::rendering::UpAxis::NEG_Y: {
    // azelToDirection = normalize({sin(az)*cos(el), sin(el), cos(az)*cos(el)})
    const vsr::math::float3 D = d;
    el_rad = std::asin(D.y);
    az_rad = std::atan2(D.x, D.z);
  ...
  case vsr::rendering::UpAxis::POS_Z: {
  ...
  case vsr::rendering::UpAxis::NEG_Z: {
    // azelToDirection = normalize({sin(az)*cos(el), cos(az)*cos(el), sin(el)})
    const vsr::math::float3 D = d;
    el_rad = std::asin(D.z);
    az_rad = std::atan2(D.x, D.y);
  ...
  case vsr::rendering::UpAxis::POS_X: {
  ...
  case vsr::rendering::UpAxis::NEG_X: {
    // azelToDirection = normalize({sin(el), cos(az)*cos(el), sin(az)*cos(el)})
    const vsr::math::float3 D = d;
    el_rad = std::asin(D.x);
    az_rad = std::atan2(D.z, D.y);
  ...
  const vsr::math::float2 newAzel{
      -vsr::math::degrees(az_rad), -vsr::math::degrees(el_rad)};
