...
void MultiDeviceViewport::buildUI()
{
  ...
  vsr::math::int2 viewportSize(_viewportSize.x, _viewportSize.y);
...
void MultiDeviceViewport::setManipulator(vsr::rendering::Manipulator *m)
{
...
void MultiDeviceViewport::resetView(bool resetAzEl)
{
  ...
  auto azel = resetAzEl ? vsr::math::float2(0.f, 20.f) : m_arcball->azel();
...
void MultiDeviceViewport::setLibrary(const std::string &libName)
{
  ...
  auto library =
      anari::loadLibrary(libName.c_str(), vsr::app::anariStatusFunc, ctx);
  if (!library) {
    vsr::core::logError(
  ...
  vsr::core::logStatus("[multi-viewport] tethering devices...");
  ...
  vsr::core::logStatus("[multi-viewport] setting up render pipeline...");

  setupImagePipeline(devices);

  vsr::core::logStatus("[multi-viewport] creating cameras and renderers...");

  adm.setRenderIndexKind(vsr::app::RenderIndexKind::ALL_LAYERS);
  for (size_t i = 0; i < scene.numberOfLayers(); i++) {
    ...
    auto *ri = (vsr::rendering::RenderIndexAllLayers *)adm.acquireRenderIndex(
        scene, libName, d);
  ...
  if (firstFrame || m_arcball->distance() == vsr::math::inf) {
    resetView(true);
    if (ctx->view.poses.empty()) {
      vsr::core::logStatus("[multi-viewport] adding 'default' camera pose");
      ctx->addCurrentViewToCameraPoses("default");
    }
    firstFrame = false;
  }

  vsr::core::logStatus("[multi-viewport] ...viewport setup complete");
...
void MultiDeviceViewport::loadSettings(vsr::core::DataNode &root)
{
  ...
  if (auto *c = root.child("camera"); c != nullptr) {
    vsr::math::float3 at(0.f);
    float distance = 0.f;
    vsr::math::float2 azel(0.f);
    int axis = 0;
    int mode = 0;
...
vsr::rendering::RenderIndexAllLayers *MultiDeviceViewport::getRenderIndex(
    size_t i) const
{
  return m_renderIndices.at(i);
}

void MultiDeviceViewport::getSceneBounds(vsr::math::float3 boundsOut[2]) const
{
  ...
  if (!anariGetProperty(d,
          ...
          sizeof(bounds),
          ANARI_WAIT)) {
    vsr::core::logWarning("[multi-viewport] ANARIWorld returned no bounds!");
  }
#endif

  std::memcpy(boundsOut, bounds, sizeof(vsr::math::float3) * 2);
}
...
void MultiDeviceViewport::reshape(vsr::math::int2 newSize)
{
  ...
  m_renderSize =
      vsr::math::int2(vsr::math::float2(m_viewportSize) * m_resolutionScale);
...
void MultiDeviceViewport::updateCamera(bool force)
{
  ...
  for (size_t i = 0; i < m_cameras.size(); i++) {
    ...
    vsr::rendering::updateCameraParametersPerspective(d, c, *m_arcball);
    anari::setParameter(
...
void MultiDeviceViewport::loadANARIRendererParameters()
{
  ...
  for (auto &name : vsr::scene::getANARIObjectSubtypes(d, ANARI_RENDERER)) {
    ...
    o = vsr::scene::parseANARIObjectInfo(d, ANARI_RENDERER, name.c_str());
  ...
  if (ImGui::BeginMenuBar()) {
    ...
    if (ImGui::BeginMenu("Camera")) {
      if (ImGui::Combo("up", &m_arcballUp, "+x\0+y\0+z\0-x\0-y\0-z\0\0")) {
        m_arcball->setAxis(static_cast<vsr::rendering::UpAxis>(m_arcballUp));
        resetView();
      }

      auto mode = static_cast<int>(m_arcball->mode());
      if (ImGui::Combo("mode", &mode, "Orbit\0Look\0\0"))
        m_arcball->setMode(static_cast<vsr::rendering::ManipulatorMode>(mode));
...
void MultiDeviceViewport::ui_handleInput()
{
  ...
  if (!anyMovement) {
    ...
    m_previousMouse = vsr::math::float2(-1);
  ...
  if (m_manipulating) {
    vsr::math::float2 position;
    std::memcpy(&position, &io.MousePos, sizeof(position));

    const vsr::math::float2 mouse(position.x, position.y);

    if (anyMovement && m_previousMouse != vsr::math::float2(-1)) {
      const vsr::math::float2 prev = m_previousMouse;

      const vsr::math::float2 mouseFrom =
          prev * 2.f / vsr::math::float2(m_viewportSize);
      const vsr::math::float2 mouseTo =
          mouse * 2.f / vsr::math::float2(m_viewportSize);

      const vsr::math::float2 mouseDelta = mouseTo - mouseFrom;

      if (mouseDelta != vsr::math::float2(0.f)) {
        if (orbit && !(pan || dolly)) {
          if (!m_mouseRotating) {
