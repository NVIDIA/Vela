...
void ExtensionManager::initialize(vsr::app::Context *ctx)
{
  m_ctx = ctx;

#ifdef VSR_USE_LUA
  m_luaContext = std::make_unique<scripting::LuaContext>();
  m_luaContext->bindScene(&ctx->vsr.scene, "scene");
  m_luaContext->bindAnimationManager(&ctx->vsr.animationMgr);

  // Route print output through vsr::core logging
  m_luaContext->setPrintCallback([](const std::string &msg) {
    std::string trimmed = msg;
    if (!trimmed.empty() && trimmed.back() == '\n')
      trimmed.pop_back();
    vsr::core::logStatus("%s", trimmed.c_str());
  });
...
void ExtensionManager::executeAction(size_t actionIndex)
{
  ...
  try {
    action.fn();
    vsr::core::logStatus("Action completed: %s", action.displayName.c_str());
