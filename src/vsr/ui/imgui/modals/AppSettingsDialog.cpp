...
void AppSettingsDialog::buildUI_applicationSettings()
{
  ...
  if (ImGui::RadioButton(
          "all layers", kind == vsr::app::RenderIndexKind::ALL_LAYERS))
    ctx->anari.setRenderIndexKind(vsr::app::RenderIndexKind::ALL_LAYERS);
  tooltipForPreviousItem("Full render index with instancing support.");
  ImGui::SameLine();
  if (ImGui::RadioButton("flat", kind == vsr::app::RenderIndexKind::FLAT))
    ctx->anari.setRenderIndexKind(vsr::app::RenderIndexKind::FLAT);
...
void AppSettingsDialog::buildUI_offlineRenderSettings()
{
  ...
  const auto &cameraDB = appContext()->vsr.scene.objectDB().camera;
  ...
  if (ImGui::Combo(
    ...
    if (currentSelection == 0)
      ctx->offline.camera.cameraIndex = VSR_INVALID_INDEX;
    else {
  ...
  auto comboGetRendererSubtype = [](void *data, int n) -> const char * {
    auto &renderers = *(std::vector<vsr::scene::Object> *)data;
  ...
  if (ImGui::Combo("AOV type", &aovIdx, aovItems, IM_ARRAYSIZE(aovItems))) {
    ctx->offline.aov.aovType = static_cast<vsr::rendering::AOVType>(aovIdx);
  }

  ImGui::BeginDisabled(
      ctx->offline.aov.aovType != vsr::rendering::AOVType::DEPTH);
  ...
  ImGui::BeginDisabled(
      ctx->offline.aov.aovType != vsr::rendering::AOVType::EDGES);
