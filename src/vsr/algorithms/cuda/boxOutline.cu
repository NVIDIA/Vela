...
{
  vsr::math::float2 s0, s1; // pixel-space endpoints
  float invW0, invW1; // 1/clip.w for perspective-correct interpolation
  vsr::math::float3 world0, world1; // world-space endpoints
};
...
ProjectedEdges projectBoxEdges(const vsr::math::box3 &box,
    const vsr::math::mat4 &projView,
    uint32_t width,
    uint32_t height)
{
  using math_float2 = vsr::math::float2;
  using math_float3 = vsr::math::float3;
  using math_float4 = vsr::math::float4;
  ...
  for (int i = 0; i < 8; i++) {
    ...
    clip[i] = vsr::math::mul(projView, math_float4(corners[i], 1.f));
  }
  ...
  for (int i = 0; i < 8; i++) {
    for (int bit = 1; bit <= 4; bit <<= 1) {
      ...
      const math_float4 c0 = vsr::math::lerp(clip[i], clip[j], t0);
      const math_float4 c1 = vsr::math::lerp(clip[i], clip[j], t1);
      ...
      edge.invW0 = 1.f / c0.w;
      edge.invW1 = 1.f / c1.w;
      edge.world0 = vsr::math::lerp(corners[i], corners[j], t0);
      edge.world1 = vsr::math::lerp(corners[i], corners[j], t1);
    }
  }
    ...
    uint32_t height)
{
  ...
  thrust::for_each(thrust::cuda::par.on(stream),
      ...
      [=] __device__(uint32_t i) {
        ...
        auto norm2 = [](const vsr::math::float2 &v) {
          return ::sqrtf(dot(v, v));
        };
        auto norm3 = [](const vsr::math::float3 &v) {
          return ::sqrtf(dot(v, v));
        };
        const vsr::math::float2 p(i % width + 0.5f, i / width + 0.5f);

        for (int e = 0; e < edges.count; e++) {
          const auto &edge = edges.e[e];
          const vsr::math::float2 d = edge.s1 - edge.s0;
          const float len2 = dot(d, d);
          const float t = len2 > 0.f
              ? ::fminf(::fmaxf(dot(p - edge.s0, d) / len2, 0.f), 1.f)
              : 0.f;
          const vsr::math::float2 closest = edge.s0 + t * d;
