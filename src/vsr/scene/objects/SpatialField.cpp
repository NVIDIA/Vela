...
SpatialField::SpatialField(Token stype) : Object(ANARI_SPATIAL_FIELD, stype)
{
  if (stype == tokens::spatial_field::structuredRegular) {
    addParameter("origin")
        .setValue(vsr::math::float3(0.f, 0.f, 0.f))
        .setDescription("bottom-left corner of the field");
    addParameter("spacing")
        .setValue(vsr::math::float3(1.f, 1.f, 1.f))
        .setMin(vsr::math::float3(0.f, 0.f, 0.f))
    ...
    addParameter("roi")
        ...
        .setValue(vsr::math::box3(
            vsr::math::float3(-math::inf, -math::inf, -math::inf),
            vsr::math::float3(math::inf, math::inf, math::inf)));
...
vsr::math::float2 SpatialField::computeValueRange()
{
  vsr::math::float2 retval{0.f, 1.f};
  auto *scene = this->scene();
  if (!scene)
    return retval;

  auto getDataRangeFromParameter =
      [&](Parameter *p) -> std::optional<vsr::math::float2> {
  ...
  if (subtype() == tokens::spatial_field::structuredRegular) {
    ...
    if (auto *range = parameter("range"); range)
      retval = range->value().get<vsr::math::float2>();
