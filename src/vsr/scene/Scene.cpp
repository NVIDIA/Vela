    ...
    Token deviceName, anari::Device d)
{
  ...
  auto subtypes = vsr::scene::getANARIObjectSubtypes(d, ANARI_RENDERER);
  ...
  for (auto &subtype : subtypes) {
    ...
    vsr::scene::parseANARIObjectInfo(*r, d, ANARI_RENDERER, subtype.c_str());
    retval.push_back(r);
  }
...
{
  vsr::core::logDebug("Removing unused context objects");
...
void Scene::defragmentObjectStorage()
{
  ...
  if (!defrag) {
    vsr::core::logStatus("No defragmentation needed");
    return;
  } else {
    vsr::core::logStatus("Defragmenting context arrays:");
    for (const auto &pair : defrags) {
      if (!pair.second.empty())
        vsr::core::logStatus("    --> %s", anari::toString(pair.first));
    }
  }
    ...
    Array::MemoryKind kind)
{
  if (items0 + items1 + items2 == 0) {
    vsr::core::logWarning("Not creating an array with zero elements");
    return {};
  }
