...
void Frame::commitParameters()
{
  ...
  m_renderer = getParamObject<VSRObject>("renderer");
  m_camera = getParamObject<VSRObject>("camera");
}
...
void Frame::finalize()
{
  ...
  anari::setParameter(d,
      m_anariFrame,
      "renderer",
      ri->renderer(m_renderer->vsrObject()->index()));
  anari::setParameter(
      d, m_anariFrame, "camera", ri->camera(m_camera->vsrObject()->index()));
  anari::setParameter(d,
      m_anariFrame,
      "world",
...
void Frame::renderFrame()
{
  ...
  if (m_lastCommitFlushOccured < state->commitBuffer.lastObjectFinalization()) {
    ...
    if (!state->usingExternalScene()) {
      const char *filename = "live_capture.vsr";
      reportMessage(
          ANARI_SEVERITY_INFO, "saving Scene Archive to '%s'", filename);
      if (!vsr::io::save_SceneArchive(*state->scene, filename)) {
...
VSR_DEVICE_ANARI_TYPEFOR_DEFINITION(vsr_device::Frame *);
