      ...
      m_instanceData(this)
{
  ...
  if (!s->usingExternalScene()) {
    m_renderIndex =
        (vsr::rendering::RenderIndexAllLayers *)s->anari.acquireRenderIndex(
...
void World::updateLayer()
{
  ...
  if (m_zeroSurfaceData || m_zeroVolumeData || m_zeroLightData) {
    ...
    for (auto *obj : m_zeroSurfaces)
      zi->insert_last_child({l, obj->vsrObject()});
...
const vsr::rendering::RenderIndexAllLayers *World::getRenderIndex() const
{
  return m_renderIndex;
}

vsr::rendering::RenderIndexAllLayers *World::getRenderIndex()
{
  return m_renderIndex;
}

vsr::scene::Layer *World::layer() const
{
  auto *s = deviceState();
...
void World::updateValidObjects()
{
  ...
  if (m_zeroSurfaceData) {
    std::for_each(m_zeroSurfaceData->handlesBegin(),
        ...
        [&](auto *o) {
          if (o)
            m_zeroSurfaces.push_back((VSRObject *)o);
        });
  }
  ...
  if (m_zeroVolumeData) {
    std::for_each(m_zeroVolumeData->handlesBegin(),
        ...
        [&](auto *o) {
          if (o)
            m_zeroVolumes.push_back((VSRObject *)o);
        });
  }
  ...
  if (m_zeroLightData) {
    std::for_each(m_zeroLightData->handlesBegin(),
        ...
        [&](auto *o) {
          if (o)
            m_zeroLights.push_back((VSRObject *)o);
        });
  }
}
...
VSR_DEVICE_ANARI_TYPEFOR_DEFINITION(vsr_device::World *);
