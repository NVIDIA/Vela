...
{
  initDevice();
  return (ANARICamera) new VSRObject(ANARI_CAMERA, deviceState(), subtype);
}
...
{
  initDevice();
  return (ANARIGeometry) new VSRObject(ANARI_GEOMETRY, deviceState(), subtype);
}
...
{
  initDevice();
  return (ANARILight) new VSRObject(ANARI_LIGHT, deviceState(), subtype);
}

ANARIMaterial Device::newMaterial(const char *subtype)
{
  initDevice();
  return (ANARIMaterial) new VSRObject(ANARI_MATERIAL, deviceState(), subtype);
}

ANARIRenderer Device::newRenderer(const char *subtype)
{
  initDevice();
  return (ANARIRenderer) new VSRObject(ANARI_RENDERER, deviceState(), subtype);
}

ANARISampler Device::newSampler(const char *subtype)
{
  initDevice();
  return (ANARISampler) new VSRObject(ANARI_SAMPLER, deviceState(), subtype);
}

ANARISpatialField Device::newSpatialField(const char *subtype)
{
  initDevice();
  return (ANARISpatialField) new VSRObject(
...
{
  initDevice();
  return (ANARISurface) new VSRObject(ANARI_SURFACE, deviceState());
}

ANARIVolume Device::newVolume(const char *subtype)
{
  initDevice();
  return (ANARIVolume) new VSRObject(ANARI_VOLUME, deviceState(), subtype);
}
...
{
  return vsr_device::query_object_types(objectType);
}
...
{
  return vsr_device::query_object_info(
...
const void *Device::getParameterInfo(ANARIDataType objectType,
    ...
    ANARIDataType infoType)
{
  return vsr_device::query_param_info(objectType,
      objectSubtype,
      parameterName,
      parameterType,
      infoName,
...
Device::~Device()
{
  ...
  reportMessage(ANARI_SEVERITY_DEBUG, "destroying VSR device (%p)", this);
}
...
void Device::initDevice()
{
  ...
  reportMessage(ANARI_SEVERITY_DEBUG, "initializing VSR device (%p)", this);
  ...
  if (m_scene != nullptr) {
    reportMessage(ANARI_SEVERITY_INFO,
        "using externally provided VSR scene (%p)",
        m_scene);
    state->scene = m_scene;
  } else {
    reportMessage(ANARI_SEVERITY_INFO,
        "using internal VSR scene (%p) and setting up a surrogate ANARI device",
        (void *)state->scene);

    auto *libraryNameEnv = getenv("ANARI_VSR_LIBRARY");
...
void Device::deviceCommitParameters()
{
  ...
  m_scene = (vsr::scene::Scene *)getParam<void *>("scene", nullptr);
}
    ...
    uint32_t flags)
{
  ...
  if (prop == "extension" && type == ANARI_STRING_LIST) {
    ...
    return 1;
  } else if (prop == "vsr" && type == ANARI_BOOL) {
...
extern "C" ANARIDevice anariNewVsrDevice(
    ANARIStatusCallback defaultCallback, const void *userPtr)
{
  return (ANARIDevice) new vsr_device::Device(defaultCallback, userPtr);
}
