...
SCENARIO("USD import options round-trip through a data tree", "[UsdImport]")
{
  GIVEN("Options differing from their defaults in every field")
  {
    ...
    WHEN("They are written to a data tree and read back")
    {
      ...
      vsr::io::UsdImportOptions restored;
      ...
      THEN("Every field survives the round-trip")
      {
        ...
        REQUIRE(restored.materialMode == vsr::io::UsdMaterialMode::MATERIALX);
...
SCENARIO("Import report counts skipped prims by reason", "[UsdImport]")
{
  ...
  report.skipped.push_back(
      {"/a", "Mesh", vsr::io::UsdSkipReason::PURPOSE_EXCLUDED, ""});
  report.skipped.push_back(
      {"/b", "Mesh", vsr::io::UsdSkipReason::PURPOSE_EXCLUDED, ""});
  report.skipped.push_back({"/c",
      "PhysicsScene",
      vsr::io::UsdSkipReason::UNSUPPORTED_PRIM_TYPE,
      ""});

  THEN("Counts are reported per reason")
  {
    REQUIRE(report.countOf(vsr::io::UsdSkipReason::PURPOSE_EXCLUDED) == 2);
    REQUIRE(report.countOf(vsr::io::UsdSkipReason::UNSUPPORTED_PRIM_TYPE) == 1);
    REQUIRE(report.countOf(vsr::io::UsdSkipReason::TEXTURE_LOAD_FAILED) == 0);
    REQUIRE(report.contains(vsr::io::UsdSkipReason::PURPOSE_EXCLUDED));
    REQUIRE_FALSE(report.contains(vsr::io::UsdSkipReason::TEXTURE_LOAD_FAILED));
  }
}
...
SCENARIO("A USD Stage's prim hierarchy is mirrored in the Layer", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_hierarchy.usda",
...
def Xform "World"
{
    ...
    {
      ImportedStage stage("vsr_test_usd_purpose.usda", purposeStage);
      ...
      {
        REQUIRE(stage.report.countOf(vsr::io::UsdSkipReason::PURPOSE_EXCLUDED)
            == 2);
      }
    ...
    WHEN("The Stage is imported asking for proxy Purpose as well")
    {
      vsr::io::UsdImportOptions options;
      options.purposes.proxy = true;
      ImportedStage stage("vsr_test_usd_purpose.usda", purposeStage, options);
...
SCENARIO("Prims resolving to invisible import as disabled nodes", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_invisible.usda",
        std::string(R"(#usda 1.0
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("Its node is disabled and the reason is reported")
      {
        ...
        REQUIRE(stage.report.countOf(
                    vsr::io::UsdSkipReason::RESOLVED_INVISIBLE)
            == 1);
      }
    }
  }
...
SCENARIO(
    "Prim types VSR cannot represent become Placeholder Nodes", "[UsdImport]")
{
  GIVEN("A Stage containing a prim type with no VSR equivalent")
  {
    ImportedStage stage("vsr_test_usd_unsupported.usda",
        std::string(R"(#usda 1.0
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("The unsupported prim is named in the report")
      {
        ...
        REQUIRE(stage.report.skipped[0].reason
            == vsr::io::UsdSkipReason::UNSUPPORTED_LIGHT_TYPE);
      }
...
def Xform "AssetB"
{
    ...
    WHEN("The import is pointed at one subtree")
    {
      ...
      ImportedStage stage("vsr_test_usd_subtree.usda", siblingAssets, options);
    ...
    "Stage framing metadata is recorded for the application", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_framing.usda",
        std::string(R"(#usda 1.0
(
    upAxis = "Z"
    metersPerUnit = 0.01
    ...
    "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_xform_reset.usda", R"(#usda 1.0
...
def Xform "Parent"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Composing parent and child lands where USD puts the child")
      {
        ...
        const auto composed = vsr::math::mul(
...
SCENARIO("Time-varying visibility is reported rather than lost", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_animated_visibility.usda", R"(#usda 1.0
(
    startTimeCode = 0
    endTimeCode = 2
)
