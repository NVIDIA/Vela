    ...
    "Light exposure and normalization reach the emitted light", "[UsdImport]")
{
  GIVEN("A sphere light with exposure and normalization set")
  {
    ...
    ImportedStage stage("vsr_test_usd_light_radiometry.usda", R"(#usda 1.0
...
def SphereLight "Lamp"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("The light's intensity accounts for both")
      {
        ...
        auto light = stage.scene.getObject<vsr::scene::Light>(0);
        REQUIRE(light);
        REQUIRE(light->subtype() == vsr::scene::tokens::light::point);
...
SCENARIO("A shaped sphere light becomes a spot light", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_spot.usda", R"(#usda 1.0
...
)
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Spot lighting survives the import")
      {
        auto light = stage.scene.getObject<vsr::scene::Light>(0);
        REQUIRE(light);
        REQUIRE(light->subtype() == vsr::scene::tokens::light::spot);
...
SCENARIO("A distant light's brightness lands on irradiance", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_distant.usda", R"(#usda 1.0
...
def DistantLight "Sun"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("It becomes a directional light carrying irradiance, not intensity")
      {
        ...
        auto light = stage.scene.getObject<vsr::scene::Light>(0);
        REQUIRE(light);
        REQUIRE(light->subtype() == vsr::scene::tokens::light::directional);
        ...
        auto color = light->parameterValueAs<vsr::math::float3>("color");
    ...
    "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_rect.usda", R"(#usda 1.0
...
def RectLight "Panel"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Its corner and edges describe the authored rectangle")
      {
        ...
        auto light = stage.scene.getObject<vsr::scene::Light>(0);
        REQUIRE(light);
        REQUIRE(light->subtype() == vsr::scene::tokens::light::quad);
        ...
        auto position = light->parameterValueAs<vsr::math::float3>("position");
        REQUIRE(position.has_value());
        REQUIRE(position->x == Approx(-2.f));
        REQUIRE(position->y == Approx(-1.f));
        REQUIRE(position->z == Approx(0.f));

        auto edge1 = light->parameterValueAs<vsr::math::float3>("edge1");
        REQUIRE(edge1.has_value());
        REQUIRE(edge1->x == Approx(4.f));
        REQUIRE(edge1->y == Approx(0.f));

        auto edge2 = light->parameterValueAs<vsr::math::float3>("edge2");
...
SCENARIO("An untextured dome light still lights the scene", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_dome.usda", R"(#usda 1.0
...
def DomeLight "Sky"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Its colour arrives baked into a constant radiance, not as color")
      {
        ...
        auto light = stage.scene.getObject<vsr::scene::Light>(0);
        REQUIRE(light);
        REQUIRE(light->subtype() == vsr::scene::tokens::light::hdri);
        ...
        REQUIRE(
            !light->parameterValueAs<vsr::math::float3>("color").has_value());
...
SCENARIO("Cameras from a Stage arrive in the camera pool", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_camera.usda", R"(#usda 1.0
(
    startTimeCode = 0
    endTimeCode = 2
)
...
def Xform "Rig"
{
    ...
    const auto camerasBefore =
        vsr::scene::Scene().numberOfObjects(ANARI_CAMERA);
