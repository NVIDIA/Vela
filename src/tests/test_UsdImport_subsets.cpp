...
SCENARIO("Per-face material subsets become several Surfaces", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_subsets.usda", R"(#usda 1.0
...
def Xform "World"
{
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("The faces no subset claims are still drawn, by the mesh's material")
      {
        ...
        REQUIRE(leftover);
        auto *index = leftover->parameterValueAsObject<vsr::scene::Array>(
            "primitive.index");
      ...
      THEN("The subset Surfaces share the mesh's vertex positions")
      {
        ...
        std::vector<const vsr::scene::Array *> positions;
        ...
        for (size_t i = 0; i < numGeometries; ++i) {
          ...
          if (!geometry)
            continue;
          if (auto *p = geometry->parameterValueAsObject<vsr::scene::Array>(
                  "vertex.position"))
...
SCENARIO("Face-varying UVs follow each material subset", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_subset_facevarying.usda", R"(#usda 1.0
...
def Xform "World"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Each subset carries the corners of the faces it selected")
      {
        ...
        auto *leftUVs = left->parameterValueAsObject<vsr::scene::Array>(
            "faceVarying.attribute0");
        auto *rightUVs = right->parameterValueAsObject<vsr::scene::Array>(
            "faceVarying.attribute0");
        ...
        const auto *l = leftUVs->dataAs<vsr::math::float2>();
        const auto *r = rightUVs->dataAs<vsr::math::float2>();
      ...
      THEN("Vertex-interpolated attributes are still shared, not copied")
      {
        ...
        auto *leftNormals =
            left->parameterValueAsObject<vsr::scene::Array>("vertex.normal");
        REQUIRE(leftNormals != nullptr);
        REQUIRE(leftNormals
            == right->parameterValueAsObject<vsr::scene::Array>(
                "vertex.normal"));
      }
    }
  }
    ...
    "Face-varying primvars survive an already-triangulated mesh", "[UsdImport]")
{
  GIVEN("An all-triangle mesh with indexed face-varying UVs and normals")
  {
    ...
    ImportedStage stage(
        "vsr_test_usd_triangulated_facevarying.usda", R"(#usda 1.0
...
def Xform "World"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("The UVs arrive flattened, one value per triangle corner")
      {
        ...
        const auto *uv = uvs->dataAs<vsr::math::float2>();
      ...
      THEN("The normals arrive too")
      {
        ...
        auto *normals = geometry->parameterValueAsObject<vsr::scene::Array>(
            "faceVarying.normal");
...
SCENARIO("A subset binds the UV primvar its own material reads", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_subset_uv_primvar.usda", R"(#usda 1.0
...
def Xform "World"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Each subset's own primvar lands on its first attribute")
      {
        ...
        REQUIRE(leftUVs != nullptr);
        REQUIRE(rightUVs != nullptr);
        REQUIRE(leftUVs->dataAs<vsr::math::float2>()[0].x == Approx(0.25f));
        REQUIRE(rightUVs->dataAs<vsr::math::float2>()[0].x == Approx(0.75f));
      }
    }
  }
}
