...
// Materials that land on VSR's portable material.
...
SCENARIO("An unconventionally named UV primvar is still found", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_uv_primvar.usda", R"(#usda 1.0
    ...
    "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_unresolvable_material.usda", R"(#usda 1.0
...
def Xform "World"
{
    ...
    {
      ImportedStage stage("vsr_test_usd_omnipbr.usda", omniPbrStage);
      ...
      THEN("Its authored inputs arrive, not the preview-surface defaults")
      {
        ...
        auto color = material->parameterValueAs<vsr::math::float3>("baseColor");
        ...
        auto emissive =
            material->parameterValueAs<vsr::math::float3>("emissive");
    ...
    WHEN("MDL passthrough is asked for instead")
    {
      ...
      ImportedStage stage("vsr_test_usd_omnipbr.usda", omniPbrStage, options);
  ...
  {
    TextureFixture diffuse("vsr_test_usd_omnipbr_diffuse.tga");

    ImportedStage stage("vsr_test_usd_omnipbr_textured.usda", R"(#usda 1.0
...
def Xform "World"
{
    def Material "OmniPBR"
    {
        ...
        def Shader "Shader"
        {
            ...
            uniform asset info:mdl:sourceAsset = @OmniPBR.mdl@
            uniform token info:mdl:sourceAsset:subIdentifier = "OmniPBR"
            asset inputs:diffuse_texture = @vsr_test_usd_omnipbr_diffuse.tga@
    ...
    WHEN("The default material mode is used")
    {
      ...
      {
        REQUIRE(
            material->parameterValueAsObject<vsr::scene::Sampler>("baseColor")
            != nullptr);
        REQUIRE(!stage.report.contains(
  ...
  {
    ImportedStage stage("vsr_test_usd_omnipbr_lookalike.usda", R"(#usda 1.0
    ...
    WHEN("The default material mode is used")
    {
      THEN("A module that merely starts with the name is not mapped as one")
      {
        ...
        auto color = boundMaterial(stage.scene)
                         ->parameterValueAs<vsr::math::float3>("baseColor");
        REQUIRE(color.has_value());
    ...
    "A prim with no bound material takes its display colour", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_display_color.usda", R"(#usda 1.0
...
def Mesh "Quad"
{
    ...
    WHEN("The Stage is imported")
    {
      ...
      {
        auto surface = stage.scene.getObject<vsr::scene::Surface>(0);
        REQUIRE(surface);
        ...
        const auto color =
            material->parameterValueAs<vsr::math::float3>("color");
        REQUIRE(color.has_value());
    ...
    "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_points_display_color.usda", R"(#usda 1.0
...
def Points "Cloud"
{
    ...
    WHEN("The Stage is imported")
    {
      THEN("Its material carries the display colour, not VSR's default")
      {
        auto surface = stage.scene.getObject<vsr::scene::Surface>(0);
        REQUIRE(surface);
        auto *material = surface->parameterValueAsObject<vsr::scene::Material>(
