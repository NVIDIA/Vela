...
SCENARIO("A point instancer shares one set of Prototype objects", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_point_instancer.usda", R"(#usda 1.0
...
def PointInstancer "Scatter"
{
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("The placements become a single transform-array node")
      {
        ...
        // placements only.
        vsr::scene::Array *transforms = nullptr;
...
SCENARIO("USD Instances share objects across placements", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_native_instance.usda", R"(#usda 1.0
...
)
{
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("Each placement is a node referencing the shared objects")
      {
        ...
        auto sharedObjectUnder = [&](vsr::scene::LayerNodeRef parent) {
          size_t index = vsr::core::INVALID_INDEX;
          layer->traverse(parent, [&](auto &node, int) {
            if (index == vsr::core::INVALID_INDEX && node->isObject())
        ...
        REQUIRE(indexA != vsr::core::INVALID_INDEX);
...
vsr::scene::Array *findTransformArray(
    vsr::scene::Layer *layer, const char *name)
{
  auto parent = findNode(layer, name);
  if (!parent)
    return nullptr;
  vsr::scene::Array *found = nullptr;
  layer->traverse(parent, [&](auto &node, int) {
...
SCENARIO("A point instancer's placements follow the Stage clock", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_animated_instancer.usda", R"(#usda 1.0
(
    startTimeCode = 0
    endTimeCode = 2
)
...
def PointInstancer "Swarm"
{
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("The imported placements are the Stage's first frame")
      {
        ...
        const auto *m = transforms->dataAs<vsr::math::mat4>();
      ...
      THEN("Scrubbing re-fills the same Array in place")
      {
        ...
        const auto *m = after->dataAs<vsr::math::mat4>();
    ...
    "An instancer whose placement count changes reallocates", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_growing_instancer.usda", R"(#usda 1.0
(
    startTimeCode = 0
    endTimeCode = 2
)
...
def PointInstancer "Growing"
{
    ...
    WHEN("The Stage is imported and scrubbed past the change")
    {
      ...
      THEN("The node is re-pointed at a right-sized Array")
      {
        ...
        REQUIRE(after->dataAs<vsr::math::mat4>()[2][3].x == Approx(2.f));
      }
    }
  }
}
...
SCENARIO("Instancer bindings survive save and reload", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_instancer_archive.usda", R"(#usda 1.0
(
    startTimeCode = 0
    endTimeCode = 2
)
...
def PointInstancer "Swarm"
{
    ...
    WHEN("The animation manager round-trips through an Archive")
    {
      ...
      REQUIRE(
          vsr::io::deserialize_AnimationManagerArchive(restored, tree.root()));
      ...
      THEN("The reconstructed binding scrubs the same Array")
      {
        ...
        auto *transforms =
            findTransformArray(stage.scene.defaultLayer(), "Swarm");
        REQUIRE(transforms != nullptr);
        REQUIRE(transforms->dataAs<vsr::math::mat4>()[1][3].x == Approx(9.f));
      }
    }
  }
}
    ...
    "A Stage that authored no time-code range still animates", "[UsdImport]")
{
  GIVEN("A PointInstancer with time samples but no startTimeCode")
  {
    ...
    ImportedStage stage("vsr_test_usd_unranged_instancer.usda", R"(#usda 1.0
...
def PointInstancer "Drifting"
{
    ...
    WHEN("The Stage is imported and scrubbed to the end")
    {
      ...
      THEN("The authored samples define the range that time maps onto")
      {
        ...
        REQUIRE(transforms != nullptr);
        REQUIRE(transforms->dataAs<vsr::math::mat4>()[1][3].x == Approx(7.f));
      }
    }
  }
  ...
  {
    ImportedStage stage("vsr_test_usd_unranged_mesh.usda", R"(#usda 1.0
...
def Mesh "Blob"
{
    ...
    WHEN("The Stage is imported and scrubbed to the end")
    {
      ...
      THEN("The authored samples define the range that time maps onto")
      {
        ...
        REQUIRE(positions->dataAs<vsr::math::float3>()[1].x == Approx(4.f));
      }
    }
  }
}
