...
namespace vsr::scivis_studio::test {
...
static ProjectSavePlan twoAssetPlan(const std::filesystem::path &root)
{
  ...
  plan.assets.push_back(textAsset(
      "first asset", "assets/first.vsr", "new first", "assets/first.vsr"));
  plan.assets.push_back(textAsset(
      "second asset", "assets/second.vsr", "new second", "assets/second.vsr"));
  plan.manifest =
      textAsset("manifest", "project.vsr", "new manifest", "project.vsr");
  return plan;
}
...
static void createOldProject(const std::filesystem::path &root)
{
  ...
  writeText(root / "assets/first.vsr", "old first");
  writeText(root / "assets/second.vsr", "old second");
  writeText(root / "project.vsr", "old manifest");
}

static void requireOldProject(const std::filesystem::path &root)
{
  REQUIRE(readText(root / "assets/first.vsr") == "old first");
  REQUIRE(readText(root / "assets/second.vsr") == "old second");
  REQUIRE(readText(root / "project.vsr") == "old manifest");
}
    ...
    "[ProjectAssetTransaction]")
{
  const auto root = std::filesystem::temp_directory_path()
      / "vsr_project_asset_transaction_success";
  std::filesystem::remove_all(root);
  createOldProject(root);

  auto plan = twoAssetPlan(root);
  writeText(root / "assets/obsolete.vsr", "obsolete");
  writeText(root / "assets/unlisted.vsr", "unlisted");
  plan.removals.push_back("assets/obsolete.vsr");
  bool firstWriterSawOldProject = false;
  plan.assets.front().writer = [&](const std::filesystem::path &stage,
                                   std::string *) {
    firstWriterSawOldProject =
        readText(root / "assets/first.vsr") == "old first"
        && readText(root / "assets/second.vsr") == "old second"
        && readText(root / "project.vsr") == "old manifest";
  ...
  REQUIRE(readText(root / "assets/first.vsr") == "new first");
  REQUIRE(readText(root / "assets/second.vsr") == "new second");
  REQUIRE(readText(root / "project.vsr") == "new manifest");
  REQUIRE_FALSE(std::filesystem::exists(root / "assets/obsolete.vsr"));
  REQUIRE(readText(root / "assets/unlisted.vsr") == "unlisted");
...
{
  const auto root = std::filesystem::temp_directory_path()
      / "vsr_project_asset_transaction_collision";
  std::filesystem::remove_all(root);
  createOldProject(root);

  auto plan = twoAssetPlan(root);
  plan.assets.front().target = "assets/FIRST.vsr";
    ...
    "[ProjectAssetTransaction]")
{
  ...
  for (size_t i = 0; i < failures.size(); ++i) {
    CAPTURE(i);
    const auto root = std::filesystem::temp_directory_path()
        / ("vsr_project_asset_transaction_phase_" + std::to_string(i));
    std::filesystem::remove_all(root);
    createOldProject(root);
...
{
  const auto root = std::filesystem::temp_directory_path()
      / "vsr_project_asset_transaction_rollback_failure";
  std::filesystem::remove_all(root);
  createOldProject(root);
