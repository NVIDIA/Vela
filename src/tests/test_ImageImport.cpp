...
// they agree. See docs/vsr-io-image-import.md for the survey they came from.
//
...
float3 texelAsFloat3(const vsr::scene::Array *image, size_t index)
{
...
const vsr::scene::Array *fixtureImage(vsr::scene::Scene &scene)
{
  for (size_t i = 0; i < scene.numberOfObjects(ANARI_SAMPLER); ++i) {
    auto sampler = scene.getObject<vsr::scene::Sampler>(i);
    if (!sampler)
      continue;
    auto *image = sampler->parameterValueAsObject<vsr::scene::Array>("image");
...
float3 sampleAsAnari(const vsr::scene::Array *image, const float2 &uv)
{
...
float2 uvAtTopOfQuad(vsr::scene::Scene &scene)
{
  for (size_t i = 0; i < scene.numberOfObjects(ANARI_GEOMETRY); ++i) {
    auto geometry = scene.getObject<vsr::scene::Geometry>(i);
    if (!geometry)
      continue;
    auto *positions =
        geometry->parameterValueAsObject<vsr::scene::Array>("vertex.position");
    auto *uvs = geometry->parameterValueAsObject<vsr::scene::Array>(
        "vertex.attribute0");
...
SCENARIO("Decoded images are stored in ANARI orientation", "[ImageImport]")
{
  ...
  {
    TempFile texture("vsr_test_orient.tga", tgaFixtureContents());

    vsr::scene::Scene scene;
    vsr::io::ImageCache cache(&scene);

    WHEN("It is imported through the shared texture path")
    {
      auto sampler =
          vsr::io::importTexture(cache, texture.path(), /*isLinear=*/true);

      THEN("Row 0 of the array is the top row of the picture")
      {
        REQUIRE(sampler);
        auto *image =
            sampler->parameterValueAsObject<vsr::scene::Array>("image");
...
SCENARIO("An imported HDRI's radiance runs bottom-up", "[ImageImport]")
{
  ...
  {
    TempFile hdri("vsr_test_orient.hdr", hdrFixtureContents());
    ...
    {
      vsr::io::import_HDRI(scene, animMgr, hdri.path().c_str());

      THEN("Row 0 of the radiance array is the bottom row of the picture")
      {
        REQUIRE(scene.numberOfObjects(ANARI_LIGHT) == 1);
        auto light = scene.getObject<vsr::scene::Light>(0);
        REQUIRE(light);
        auto *radiance =
            light->parameterValueAsObject<vsr::scene::Array>("radiance");
    ...
    "[ImageImport]")
{
  ...
  {
    TempFile texture("vsr_test_orient.dds", ddsFixtureContents());
    ...
    {
      auto sampler = vsr::io::importTexture(cache, texture.path());

      THEN("The sampler leaves the coordinates alone")
      {
        REQUIRE(sampler);
        REQUIRE(sampler->subtype()
            == vsr::scene::tokens::sampler::compressedImage2D);
    ...
    WHEN("It is imported by a caller that authored its own uv transform")
    {
      ...
      vsr::io::SamplerSettings settings;
      settings.uvTransform =
          vsr::io::UvTransform{vsr::core::math::mat4(float4(1.f, 0.f, 0.f, 0.f),
              float4(0.f, 0.5f, 0.f, 0.f),
              float4(0.f, 0.f, 1.f, 0.f),
              float4(0.f, 0.f, 0.f, 1.f))};

      auto sampler = vsr::io::importTexture(
    ...
    WHEN("It is acquired for a consumer that wants the opposite row order")
    {
      auto image = cache.acquire({texture.path(),
          vsr::io::ColorSpace::SRGB,
          vsr::io::RowOrder::BOTTOM_UP});
      auto sampler = vsr::io::makeImageSampler(cache, image, texture.path());
    ...
    "[ImageImport]")
{
  ...
  {
    TempFile texture("vsr_test_orient.tga", tgaFixtureContents());
    TempFile mtl("vsr_test_orient.mtl", mtlContents("vsr_test_orient.tga"));
    TempFile obj("vsr_test_orient.obj", objContents("vsr_test_orient.mtl"));
    ...
    WHEN("It is imported")
    {
      vsr::io::import_OBJ(scene, animMgr, obj.path().c_str());
  ...
  {
    TempFile texture("vsr_test_orient.tga", tgaFixtureContents());
    TempFile bin("vsr_test_orient.bin", gltfBufferContents());
    TempFile gltf("vsr_test_orient.gltf",
        gltfContents("vsr_test_orient.bin", "vsr_test_orient.tga"));
  ...
  {
    TempFile texture("vsr_test_orient.tga", tgaFixtureContents());
    TempFile pbrt("vsr_test_orient.pbrt", pbrtContents("vsr_test_orient.tga"));
    ...
    WHEN("It is imported")
    {
      vsr::io::import_PBRT(scene, animMgr, pbrt.path().c_str());
  ...
  {
    TempFile texture("vsr_test_orient.tga", tgaFixtureContents());
    TempFile stage("vsr_test_orient.usda", usdContents("vsr_test_orient.tga"));
    ...
    WHEN("It is imported")
    {
      vsr::io::import_USD(scene, animMgr, stage.path().c_str());
