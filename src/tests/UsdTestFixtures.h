...
// only from inside a `#if VSR_USE_USD` guard -- it needs OpenUSD-dependent
// declarations.
...
inline vsr::scene::LayerNodeRef findNode(
    vsr::scene::Layer *layer, const char *name)
{
  vsr::scene::LayerNodeRef found;
...
// names it after.
template <typename T>
vsr::core::ObjectPoolRef<T> findObject(
    vsr::scene::Scene &scene, anari::DataType type, const char *name)
{
...
inline vsr::scene::GeometryRef findGeometry(
    vsr::scene::Scene &scene, const char *name)
{
  return findObject<vsr::scene::Geometry>(scene, ANARI_GEOMETRY, name);
}
...
inline vsr::scene::Material *boundMaterial(vsr::scene::Scene &scene)
{
  auto surface = scene.getObject<vsr::scene::Surface>(0);
  REQUIRE(surface);
...
{
  report = vsr::io::import_USD(scene, animMgr, path().c_str(), {}, options);
}
