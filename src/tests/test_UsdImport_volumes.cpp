...
vsr::math::float2 valueRangeOf(vsr::scene::Volume *volume)
{
  auto *parameter = volume->parameter("valueRange");
  REQUIRE(parameter != nullptr);
  return parameter->value().getAs<vsr::math::float2>(ANARI_FLOAT32_BOX1);
}
...
SCENARIO("A UsdVol Volume imports the field it references", "[UsdImport]")
{
  ...
  {
    RawFieldFixture field("vsr_test_usd_field_2x2x2_uint8.raw");
    ImportedStage stage("vsr_test_usd_volume.usda", R"(#usda 1.0
...
def Volume "Vol"
{
    ...
    def "Density"
    {
        asset filePath = @vsr_test_usd_field_2x2x2_uint8.raw@
    }
}
)");
    ...
    WHEN("The Stage is imported")
    {
      THEN("The volume carries the field and a default color map")
      {
        ...
        auto volume =
            findObject<vsr::scene::Volume>(stage.scene, ANARI_VOLUME, "/Vol");
        REQUIRE(volume);
        REQUIRE(
    ...
    "[UsdImport]")
{
  ...
  {
    RawFieldFixture field("vsr_test_usd_annotated_2x2x2_uint8.raw");
    ImportedStage stage("vsr_test_usd_volume_annotated.usda", R"(#usda 1.0
...
def Volume "Vol"
{
    ...
    def "Density"
    {
        asset filePath = @vsr_test_usd_annotated_2x2x2_uint8.raw@
    }
}
)");
    ...
    "[UsdImport]")
{
  ...
  {
    RawFieldFixture field("vsr_test_usd_colormapped_2x2x2_uint8.raw");
    ImportedStage stage("vsr_test_usd_volume_colormap.usda", R"(#usda 1.0
...
def Volume "Vol"
{
    ...
    def "Density"
    {
        asset filePath = @vsr_test_usd_colormapped_2x2x2_uint8.raw@
    }
    ...
    WHEN("The Stage is imported")
    {
      THEN("The colormap's colors and domain reach the volume")
      {
        ...
        REQUIRE(color != nullptr);
        REQUIRE(color->size() == 256);
        const auto *texels = color->dataAs<vsr::math::float4>();
...
SCENARIO("A Volume whose field cannot be loaded is reported", "[UsdImport]")
{
  ...
  {
    ImportedStage stage("vsr_test_usd_volume_missing_field.usda", R"(#usda 1.0
...
def Volume "Vol"
{
    ...
    def "Density"
    {
        asset filePath = @vsr_test_usd_absent_2x2x2_uint8.raw@
    }
}
)");
    ...
    WHEN("The Stage is imported")
    {
      ...
      THEN("The prim is named in the Import Report")
      {
        ...
        REQUIRE(stage.report.skipped[0].reason
            == vsr::io::UsdSkipReason::FIELD_LOAD_FAILED);
        REQUIRE(stage.report.skipped[0].detail.find(
                    "vsr_test_usd_absent_2x2x2_uint8.raw")
            != std::string::npos);
      }
    ...
    "[UsdImport]")
{
  ...
  {
    UnreadableFieldFixture field("vsr_test_usd_field.silo");
    LogCapture log;
    ImportedStage stage("vsr_test_usd_volume_silo.usda", R"(#usda 1.0
...
def Volume "Vol"
{
    ...
    def "Density"
    {
        asset filePath = @vsr_test_usd_field.silo@
    }
}
)");
  ...
  {
    UnreadableFieldFixture field("vsr_test_usd_field.hdf5");
    LogCapture log;
    ImportedStage stage("vsr_test_usd_volume_hdf5.usda", R"(#usda 1.0
...
def Volume "Vol"
{
    ...
    def "Density"
    {
        asset filePath = @vsr_test_usd_field.hdf5@
    }
}
)");
