    ...
    "[ComponentSerialization]")
{
  ...
  auto sourceObject = sourceScene.createObject<vsr::scene::Geometry>("sphere");
  auto &source = sourceObject->addParameter("attribute");
  source.setValue(0.5f)
      .setDescription("selected attribute")
      .setUsage(vsr::scene::ParameterUsageHint::COLOR)
      .setMin(0.f)
      .setMax(1.f)
  ...
  auto &node = tree.root();
  vsr::io::serialize_Parameter(source, node);
  ...
  REQUIRE(node["usage"].getValueAs<int>()
      == int(vsr::scene::ParameterUsageHint::COLOR));
  ...
  REQUIRE(node.child("__vsr_metadata") == nullptr);

  vsr::scene::Scene targetScene;
  auto targetObject = targetScene.createObject<vsr::scene::Geometry>("sphere");
  auto &target = targetObject->addParameter("attribute");
  vsr::io::deserialize_Parameter(node, target);

  REQUIRE(target.value().getAs<float>() == 0.5f);
  REQUIRE_FALSE(target.isEnabled());
  REQUIRE(target.description() == "selected attribute");
  REQUIRE(target.usage() == vsr::scene::ParameterUsageHint::COLOR);
...
{
  vsr::scene::Scene sourceScene;
  auto source = sourceScene.createObject<vsr::scene::Geometry>("sphere");
  ...
  vsr::core::DataTree tree;
  auto &node = tree.root();
  vsr::io::serialize_Object(*source, node);
  ...
  vsr::scene::Scene targetScene;
  vsr::io::deserialize_Object(targetScene, node);
  auto target = targetScene.getObject<vsr::scene::Geometry>(source->index());
    ...
    "[ComponentSerialization]")
{
  ...
  auto group = sourceScene.insertChildTransformNode(
      sourceLayer->root(), vsr::math::IDENTITY_MAT4, "group");
  ...
  vsr::core::DataTree tree;
  auto &node = tree.root();
  vsr::io::serialize_Layer(*sourceLayer, node);
  ...
  vsr::scene::Scene targetScene;
  targetScene.createObject<vsr::scene::Geometry>("sphere");
  auto *targetLayer = targetScene.defaultLayer();
  vsr::io::deserialize_Layer(node, *targetLayer, targetScene);

  bool sawGroup = false;
  bool sawInstance = false;
  targetLayer->traverse(
      targetLayer->root(), [&](vsr::scene::LayerNode &layerNode, int level) {
        if (level == 1) {
          sawGroup = true;
