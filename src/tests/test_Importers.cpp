...
SCENARIO("Splitting a path names its file and its directory", "[Importers]")
{
  ...
  auto rejoins = [](const char *path) {
    return vsr::io::pathOf(path) + vsr::io::fileOf(path) == path;
  };
  ...
  GIVEN("A path with a directory component")
  {
    ...
    {
      REQUIRE(vsr::io::fileOf("/a/b/volume.raw") == "volume.raw");
      REQUIRE(vsr::io::pathOf("/a/b/volume.raw") == "/a/b/");
      REQUIRE(rejoins("/a/b/volume.raw"));
      REQUIRE(vsr::io::fileOf("b/volume.raw") == "volume.raw");
      REQUIRE(vsr::io::pathOf("b/volume.raw") == "b/");
  ...
  GIVEN("A bare filename, as typed relative to the working directory")
  {
    THEN("It is the file, and there is no directory")
    {
      ...
      REQUIRE(vsr::io::fileOf("volume.raw") == "volume.raw");
      REQUIRE(vsr::io::pathOf("volume.raw").empty());
  ...
  GIVEN("A path that names a directory rather than a file")
  {
    ...
    {
      REQUIRE(vsr::io::fileOf("/a/b/").empty());
      REQUIRE(vsr::io::pathOf("/a/b/") == "/a/b/");
      REQUIRE(vsr::io::fileOf("").empty());
      REQUIRE(vsr::io::pathOf("").empty());
    }
  }
  ...
  GIVEN("A file directly under the root")
  {
    ...
    {
      REQUIRE(vsr::io::fileOf("/volume.raw") == "volume.raw");
      REQUIRE(vsr::io::pathOf("/volume.raw") == "/");
      REQUIRE(rejoins("/volume.raw"));
    }
  }
}
    ...
    "[Importers]")
{
  ...
  const char *name = "vsr_test_relative_2x2x2_uint8.raw";
  {
...
{
  const auto path =
      std::filesystem::temp_directory_path() / "vsr_test_1x1x1_uint8.raw";
  {
  ...
  {
    vsr::io::import_file(
        scene, animMgr, {vsr::io::ImporterType::VOLUME, path.string()});

    THEN("The imported volume has a sampled color array")
    {
      REQUIRE(scene.numberOfObjects(ANARI_VOLUME) == 1);
      auto volume = scene.getObject<vsr::scene::Volume>(0);
      REQUIRE(volume);
      auto *color = volume->parameterValueAsObject<vsr::scene::Array>("color");
...
SCENARIO("The spatial field dispatcher reads NanoVDB under both of its names",
    "[Importers]")
{
  // vsrVolumeToNanoVDB documents its output as '.vdb', so a NanoVDB grid
  // reaches VSR under that name as often as under '.nvdb', and both have to
  // find the same reader.
  const auto rawPath =
      std::filesystem::temp_directory_path() / "vsr_test_2x2x2_uint8.raw";
  {
  ...
  const auto vdbPath =
      std::filesystem::temp_directory_path() / "vsr_test_roundtrip.vdb";
  ...
  {
    auto source =
        vsr::io::import_spatial_field(scene, rawPath.string().c_str());
    REQUIRE(source);
    vsr::io::export_StructuredVolumeToNanoVDB(source.data(), vdbPath.string());
    REQUIRE(std::filesystem::exists(vdbPath));

    WHEN("The '.vdb' file is dispatched")
    {
      auto field =
          vsr::io::import_spatial_field(scene, vdbPath.string().c_str());

      THEN("The NanoVDB reader loads it")
      {
        REQUIRE(field);
        REQUIRE(field->subtype() == vsr::scene::tokens::spatial_field::nanovdb);
      }
    }
  }
    ...
    "[Importers]")
{
  ...
  const auto path =
      std::filesystem::temp_directory_path() / "vsr_test_not_a_grid.vdb";
  ...
  GIVEN("An empty file under a '.vdb' name")
  {
    ...
    {
      REQUIRE(!vsr::io::import_spatial_field(scene, path.string().c_str()));
    }
  }
...
SCENARIO("TIFF textures decode into float texel arrays", "[Importers]")
{
  ...
  {
    TiffFixture tiff("vsr_test_1x1_rgb8.tif");
    ...
    {
      auto sampler =
          vsr::io::importTexture(cache, tiff.path(), /*isLinear=*/true);
      ...
      THEN("The sampler carries the file's texels untransformed")
      {
        ...
        const auto *texels = image->dataAs<vsr::core::math::float3>();
    ...
    {
      auto sampler =
          vsr::io::importTexture(cache, tiff.path(), /*isLinear=*/false);
  ...
  {
    GreyAlphaTiffFixture tiff("vsr_test_1x1_greyalpha.tif");
    ...
    WHEN("It is imported as an sRGB texture")
    {
      ...
      THEN("Only the grey channel is gamma-decoded, leaving alpha linear")
      {
        ...
        const auto *texels = image->dataAs<vsr::core::math::float2>();
        REQUIRE(texels[0].x == Approx(std::pow(0x60 / 255.f, 2.2f)));
        REQUIRE(texels[0].y == Approx(0x40 / 255.f));
      }
    }
