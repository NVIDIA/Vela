// SPDX-FileCopyrightText: Copyright (c) 2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
// SPDX-License-Identifier: Apache-2.0

// std
#include <iostream>
#include <memory>
#include <thread>
// vsr_network
#include <vsr/network/NetworkChannel.hpp>
// vsr_core
#include <vsr/core/Logging.hpp>
// stb_image
#include "stb_image_write.h"

#include "../RenderSession.hpp"

using vsr::network::MessageType;

int main()
{
  // Global setup //

  vsr::core::setLogToStdout();

  bool running = false;
  int framesReceived = 0;
  std::string filename = "server_test_frame.png";

  stbi_flip_vertically_on_write(1);

  vsr::network::MessagePayload frameData;

  // Client setup //

  auto session = std::make_shared<vsr::network::RenderSession>();
  auto client = std::make_shared<vsr::network::NetworkClient>();

  // Handlers //

  client->registerHandler(
      MessageType::ERROR, [](const vsr::network::Message &msg) {
        vsr::core::logError("[Client] Received error from server: '%s'",
            vsr::network::payloadAs<char>(msg));
      });

  client->registerHandler(
      MessageType::PING, [](const vsr::network::Message &msg) {
        vsr::core::logStatus("[Client] Received PING from server");
      });

  client->registerHandler(MessageType::CLIENT_RECEIVE_FRAME_CONFIG,
      [session](const vsr::network::Message &msg) {
        auto pos = 0u;
        vsr::network::payloadRead(msg, pos, &session->frame.config);
        vsr::core::logStatus("[Client] Received frame config: size=(%u,%u)",
            session->frame.config.size.x,
            session->frame.config.size.y);
      });

  client->registerHandler(MessageType::CLIENT_RECEIVE_CURRENT_CAMERA,
      [session](const vsr::network::Message &msg) {
        size_t idx = 0;
        uint32_t pos = 0;
        if (vsr::network::payloadRead(msg, pos, &idx)) {
          vsr::core::logStatus(
              "[Client] Received current camera index %u from server.", idx);
        } else {
          vsr::core::logError(
              "[Client] Invalid payload for CLIENT_RECEIVE_CURRENT_CAMERA");
        }
      });

  client->registerHandler(MessageType::CLIENT_RECEIVE_FRAME_BUFFER_COLOR,
      [&](const vsr::network::Message &msg) {
        vsr::core::logStatus(
            "[Client] Received color buffer #%i of size %u bytes",
            framesReceived++,
            msg.header.payload_length);
        frameData = msg.payload;
      });

  // Run client //

  client->connect("127.0.0.1", 12345);

  client->send(MessageType::SERVER_START_RENDERING);

  vsr::core::logStatus("[Client] Rendering for 1 second...");
  std::this_thread::sleep_for(std::chrono::seconds(1));

  client->send(MessageType::SERVER_STOP_RENDERING).get();
  client->send(MessageType::SERVER_REQUEST_FRAME_CONFIG).get();
  client->send(MessageType::SERVER_REQUEST_CURRENT_CAMERA).get();

  for (int i = 0; i < 3; ++i) {
    vsr::core::logStatus("[Client] Sending PING #%d", i + 1);
    std::this_thread::sleep_for(std::chrono::seconds(1));
    client->send(MessageType::PING);
  }

  vsr::core::logStatus("[Client] Waiting for 3 seconds...");
  std::this_thread::sleep_for(std::chrono::seconds(3));

  // Shutdown //

  vsr::core::logStatus("[Client] Sending SHUTDOWN");
  client->send(MessageType::SERVER_SHUTDOWN).get();
  client->disconnect();

  // Store fine frame //

  auto size = session->frame.config.size;

  // Write PNG file (4 components = RGBA, stride = width * 4 bytes)
  int result = stbi_write_png(filename.c_str(),
      static_cast<int>(size.x),
      static_cast<int>(size.y),
      4, // RGBA
      frameData.data(),
      static_cast<int>(size.x) * 4);

  if (result) {
    vsr::core::logStatus("[Client] Saved image to '%s'", filename.c_str());
  } else {
    vsr::core::logWarning(
        "[Client] Failed to save image to '%s'", filename.c_str());
  }

  return 0;
}
