...
void RemoteViewport::buildUI()
{
  ...
  if (!m_wasConnected && isConnected) {
    m_receivedCameraIdx = VSR_INVALID_INDEX;
...
void RemoteViewport::setManipulator(vsr::rendering::Manipulator *m)
{
  m_camera.arcball = m ? m : &m_camera.localArcball;
}

void RemoteViewport::setNetworkChannel(vsr::network::NetworkChannel *c)
{
  m_channel = c;
  ...
  c->registerHandler(MessageType::CLIENT_RECEIVE_CURRENT_RENDERER,
      [this](const vsr::network::Message &msg) {
        auto rendererIdx = *vsr::network::payloadAs<size_t>(msg);
        vsr::core::logDebug(
  ...
  c->registerHandler(MessageType::CLIENT_RECEIVE_CURRENT_CAMERA,
      [this](const vsr::network::Message &msg) {
        auto cameraIdx = *vsr::network::payloadAs<size_t>(msg);
        vsr::core::logDebug(
...
void RemoteViewport::viewport_reshape(vsr::math::int2 _newSize)
{
  ...
  auto newSize = vsr::math::uint2(_newSize.x, _newSize.y);
...
void RemoteViewport::updateRenderer()
{
  ...
  if (m_renderers.objects.empty()
    ...
    auto fr = scene.getObject<vsr::scene::Renderer>(0);
  ...
  const bool rendererFromServer =
      !m_renderers.current && m_receivedRendererIdx != VSR_INVALID_INDEX;
...
void RemoteViewport::updateCamera()
{
  ...
  const bool cameraFromServer =
      !m_camera.current && m_receivedCameraIdx != VSR_INVALID_INDEX;
  ...
  if (cameraFromServer) {
    auto *ctx = appContext();
    auto &scene = ctx->vsr.scene;
    camera_setCurrent(scene.getObject<vsr::scene::Camera>(m_receivedCameraIdx));
