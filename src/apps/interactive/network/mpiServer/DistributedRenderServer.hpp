 ...
 * interface as vsrServer (port 12345).  Rank 0 is the network-facing host;
 ...
 *
 * Build requirements: VSR_USE_MPI=ON and VSR_USE_NETWORKING=ON.
 */
struct DistributedRenderServer
{
 ...
 private:
  ...
  struct ControlState
  {
    vsr::math::int2 frameSize{800, 600};
