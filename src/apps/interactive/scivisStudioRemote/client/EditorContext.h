 ...
 * connection (Project Replica in, Project Ops out) and where to surface
 * errors and status. Errors from replies go to the Log window and a
 * transient toast, never a modal that blocks. Owned by the Application and
 * outlives every window.
 *
 * Example:
 *   if (m_context->canSend()) {
 *     RemoveShot remove;
 *     remove.shotId = shotId;
 *     m_context->ops().send(remove, m_context->errorReporter());
 *   }
 */
