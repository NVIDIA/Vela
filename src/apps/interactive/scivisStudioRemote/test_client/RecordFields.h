 ...
 * runs its set, dump-project prints every row that has a Dump form, and the
 * assert values --help and the README list are derived from the names. A
 * field is written once, here. The tables below are the Project Replica's
 * records; NamedValues.cpp keeps the session-side ones (a task, a frame
 * header, a pick, a histogram) to itself.
 *
 * Example:
 *   std::string error;
...
struct Field
{
  ...
  bool (*set)(T &, const std::string &value, std::string &error){nullptr};
  Dump dump{Dump::Plain};
};

extern const std::vector<Field<Project>> PROJECT_FIELDS;
extern const std::vector<Field<Shot>> SHOT_FIELDS;
...
template <typename T>
const Field<T> *findField(
    const std::vector<Field<T>> &fields, const std::string &name);

// The names of every row, and of every row that has a set.
template <typename T>
std::vector<std::string> fieldNames(const std::vector<Field<T>> &fields);
template <typename T>
...
template <typename T>
std::optional<std::string> fieldText(const std::vector<Field<T>> &fields,
    const T &record,
...
template <typename T>
std::string dumpFields(const std::vector<Field<T>> &fields, const T &record);
...
// One `field=value` edit of update-shot: a SHOT_FIELDS row's set, or
// `binding.<datasetId>=on|off`. False with the reason: an unknown or
// read-only field, or a value the field cannot hold.
...
template <typename T>
inline const Field<T> *findField(
    const std::vector<Field<T>> &fields, const std::string &name)
{
...
template <typename T>
inline std::vector<std::string> fieldNames(const std::vector<Field<T>> &fields)
{
...
template <typename T>
inline std::optional<std::string> fieldText(const std::vector<Field<T>> &fields,
    const T &record,
    const std::string &name,
    const char *label,
    std::string &error)
...
inline std::string dumpFields(
    const std::vector<Field<T>> &fields, const T &record)
{
  std::string out;
  for (const auto &field : fields) {
