...
{
  {
    ...
    m_ctx = std::make_unique<vsr::app::Context>();
    m_ctx->vsr.scene.setMpiRankInfo(rank, numRanks);
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "-ptc") {
      ...
      argv[i] = nullptr; // remove from args passed to vsr::app::Context
  ...
  std::vector<vsr::app::DeviceInitParam> deviceParameters;
  if (m_anari.gpusPerNode > 1) {
    auto cudaDeviceId = int(rank() % m_anari.gpusPerNode);
    deviceParameters.push_back({"cudaDevice", vsr::core::Any(cudaDeviceId)});
    deviceParameters.push_back({"dataGroupID", vsr::core::Any(-1)});
  }
  ...
  auto mat = m_ctx->vsr.scene.defaultMaterial();
  mat->setParameter(
      "color", vsr::math::float3(dist(rng), dist(rng), dist(rng)));
...
{
  if (!m_mpiInitialized) {
    vsr::core::logError("DistributedSceneController not initialized!\n");
    return;
  }
...
vsr::app::Context *DistributedSceneController::appContext()
{
  return m_ctx.get();
}
...
void DistributedSceneController::executeFrame_syncFrame()
{
  ...
  if (auto d = anariDevice(); d && m_distributedState->frame.sync()) {
    ...
    auto f = anariFrame();
    anari::setParameter(d, f, "size", vsr::math::uint2(state->size));
    anari::commitParameters(d, f);
...
void DistributedSceneController::executeFrame_syncAnimation()
{
  ...
  if (auto d = anariDevice(); d && m_distributedState->animation.sync()) {
    ...
    auto *ctx = appContext();
    ctx->vsr.animationMgr.setAnimationTime(state->time);
