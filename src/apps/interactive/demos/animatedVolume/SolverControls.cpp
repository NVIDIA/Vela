...
void SolverControls::setField(vsr::scene::SpatialFieldRef f)
{
  m_field = f;
  resetSolver();
}
...
{
  if (!m_field) {
    vsr::core::logWarning("[jacobi solver] no spatial field set");
    return;
  }
  ...
  m_field->setParameter("spacing", 2.f / vsr::math::float3(m_dims.x));
}
...
void SolverControls::resetSolver()
{
  ...
  vsr::core::logStatus("[jacobi solver] resetting solver data");
...
void SolverControls::iterateSolver()
{
  ...
  if (m_useGPUInterop) {
    ...
    vsr::demo::jacobi3D(nx, ny, nz, d_grid_1, d_grid_2, m_iterationsPerCycle);
    m_dataCUDA_1->unmap();
    m_dataCUDA_2->unmap();
    ...
    vsr::demo::jacobi3D(nx, ny, nz, h_grid, m_iterationsPerCycle);
    m_dataHost->unmap();
  }
  ...
  vsr::core::logStatus("[jacobi solver] ...solver done (%i|%.2fms)",
      m_totalIterations,
      duration);
  ...
  vsr::core::logStatus("[jacobi solver] ...VSR update  (%i|%.2fms)",
      m_totalIterations,
      duration);
...
void SolverControls::exportRAW()
{
  ...
  vsr::core::logStatus("[jacobi solver] exported data to %s", location.c_str());
}
