...
class Application : public VSRApplication
{
 public:
  ...
  vsr::ui::imgui::WindowArray setupWindows() override
  {
    ...
    auto *viewport = new vsr_ui::Viewport(this, manipulator, "Viewport");
    auto *viewport2 = new vsr_ui::Viewport(this, manipulator, "Secondary View");
    viewport2->hide();
    auto *oeditor = new vsr_ui::ObjectEditor(this);
    auto *otree = new vsr_ui::LayerTree(this);
    auto *solver = new vsr::demo::SimulationControls(this);
    ...
    auto particles = scene.createObject<vsr::scene::Geometry>(
        vsr::scene::tokens::geometry::sphere);
    particles->setName("particle_geometry");
    particles->setParameter("radius", 0.01f);

    auto blackHoles = scene.createObject<vsr::scene::Geometry>(
        vsr::scene::tokens::geometry::sphere);
    ...
    auto *colorMapPtr = samplerImageArray->mapAs<vsr::math::float4>();
    colorMapPtr[0] = vsr::math::float4(0.f, 0.f, 1.f, 1.f);
    colorMapPtr[1] = vsr::math::float4(0.f, 1.f, 0.f, 1.f);
    colorMapPtr[2] = vsr::math::float4(1.f, 0.f, 0.f, 1.f);
    samplerImageArray->unmap();

    auto sampler = scene.createObject<vsr::scene::Sampler>(
    ...
    auto particleMat = scene.createObject<vsr::scene::Material>(
        vsr::scene::tokens::material::matte);
    particleMat->setParameterObject("color", *sampler);

    auto bhMat = scene.createObject<vsr::scene::Material>(
        vsr::scene::tokens::material::matte);
    bhMat->setParameter("color", vsr::math::float3(0.f));
...
{
  {
    ...
    app.run(1920, 1080, "VSR Demo | Animated Particles");
  }
