...
SimulationControls::SimulationControls(
    vsr::ui::imgui::Application *app, const char *name)
    : vsr::ui::imgui::Window(app, name)
{}
...
void SimulationControls::setGeometry(vsr::scene::GeometryRef particles,
    vsr::scene::GeometryRef blackHoles,
    vsr::scene::SamplerRef sampler)
{
...
void SimulationControls::resetSimulation()
{
  ...
  vsr::core::logStatus("[particle system] resetting simulation data");
  ...
  auto *velocities = m_dataVelocities->mapAs<vsr::math::float3>();
  if (m_randomizeInitialVelocities) {
    ...
    std::fill(velocities, velocities + numParticles, vsr::math::float3(0.f));
  }

  auto *distances = m_dataDistances->mapAs<float>();

  auto *positions = m_dataPoints->mapAs<vsr::math::float3>();
  ...
  for (int x = 0; x < m_particlesPerSide; x++) {
    for (int y = 0; y < m_particlesPerSide; y++) {
      for (int z = 0; z < m_particlesPerSide; z++) {
        auto p = vsr::math::float3(d * x - 1.f, d * y - 1.f, d * z - 1.f);
        positions[i] = p;
        distances[i++] = vsr::math::length(p);
      }
    }
  }
...
void SimulationControls::updateColorMapScale()
{
  m_particleColorSampler->setParameter("inTransform",
      vsr::math::makeValueRangeTransform(
          0.f, m_params.maxDistance / m_colorMapScaleFactor));
}

std::pair<vsr::math::float3, vsr::math::float3>
SimulationControls::updateBhPoints()
{
  const auto rot = vsr::math::rotation_matrix(
      vsr::math::rotation_quat(vsr::math::float3(0, 0, 1), m_angle));
  vsr::math::float4 bh1_ =
      vsr::math::mul(rot, vsr::math::float4(5.f, 0.f, 0.f, 1.f));
  vsr::math::float4 bh2_ =
      vsr::math::mul(rot, vsr::math::float4(-5.f, 0.f, 0.f, 1.f));

  vsr::math::float3 bh1(bh1_.x, bh1_.y, bh1_.z);
  vsr::math::float3 bh2(bh2_.x, bh2_.y, bh2_.z);

  auto *bhPoints = m_dataBhPoints->mapAs<vsr::math::float3>();
  bhPoints[0] = bh1;
  bhPoints[1] = bh2;
...
{
  if (!m_particleGeom) {
    vsr::core::logWarning("[particle system] no geometry set, unable to run");
    return;
  }
  ...
  vsr::core::logStatus("[particle system] running time step");
  vsr::core::logStatus("    angle: %f", m_angle);

  auto *pointsCUDA = m_dataPointsCUDA->mapAs<vsr::math::float3>();
  auto *velocitiesCUDA = m_dataVelocitiesCUDA->mapAs<vsr::math::float3>();
  auto *distancesCUDA = m_dataDistancesCUDA->mapAs<float>();

  vsr::demo::particlesComputeTimestep(numParticles,
      pointsCUDA,
      velocitiesCUDA,
      distancesCUDA,
      vsr::math::float3(bh1.x, bh1.y, bh1.z),
      vsr::math::float3(bh2.x, bh2.y, bh2.z),
      m_params);

  if (!m_useGPUInterop) {
    auto *points = m_dataPoints->mapAs<vsr::math::float3>();
    auto *velocities = m_dataVelocities->mapAs<vsr::math::float3>();
    auto *distances = m_dataDistances->mapAs<float>();
