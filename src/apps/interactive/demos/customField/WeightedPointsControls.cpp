...
void WeightedPointsControls::buildUI()
{
  ...
  if (ImGui::CollapsingHeader("Lighting")) {
    if (m_light)
      vsr::ui::buildUI_object(*m_light, appContext()->vsr.scene, true);
  }
  ...
  if (ImGui::CollapsingHeader("Animation")) {
    ...
    auto &mgr = appContext()->vsr.animationMgr;
...
void WeightedPointsControls::createScene()
{
  ...
  const vsr::core::Token wpToken("weightedPoints");
  m_field = scene.createObject<vsr::scene::SpatialField>(wpToken);
  m_field->setName("weighted_points_field");
  m_field->setParameter("domainMin", vsr::math::float3(0.f, 0.f, 0.f));
  m_field->setParameter("domainMax", vsr::math::float3(1.f, 1.f, 1.f));
  m_field->setParameter("sigma", 1.f);
  m_field->setParameter("cutoff", 1.f);

  m_volume = scene.createObject<vsr::scene::Volume>(
  ...
  vsr::math::float2 valueRange(0.f, 1.f);
  ...
  auto *tfData = colorArray->mapAs<vsr::math::float4>();
  for (size_t i = 0; i < TF_SIZE; i++) {
    ...
    float a = t;
    tfData[i] = vsr::math::float4(r, g, b, a);
  }
  colorArray->unmap();
...
void WeightedPointsControls::rebuildField()
{
  ...
  if (numNodes == 0) {
    vsr::core::logStatus("[customField demo] octree produced no nodes");
    return;
  }
  ...
  m_field->setParameter(
      "domainMin", vsr::math::float3(domainMin[0], domainMin[1], domainMin[2]));
  m_field->setParameter(
      "domainMax", vsr::math::float3(domainMax[0], domainMax[1], domainMax[2]));
  ...
  vsr::math::float2 valueRange(0.f, rangeMax);
  m_volume->setParameter("valueRange", ANARI_FLOAT32_BOX1, &valueRange);
...
void WeightedPointsControls::rebuildFieldFast()
{
  ...
  m_field->setParameter(
      "domainMin", vsr::math::float3(dmin[0], dmin[1], dmin[2]));
  m_field->setParameter(
      "domainMax", vsr::math::float3(dmax[0], dmax[1], dmax[2]));
  ...
  vsr::math::float2 valueRange(0.f, maxFieldVal > 0.f ? maxFieldVal : 1.f);
  m_volume->setParameter("valueRange", ANARI_FLOAT32_BOX1, &valueRange);
...
std::vector<float> WeightedPointsControls::loadPDB(const std::string &path)
{
  ...
  if (!fp) {
    vsr::core::logWarning("[PDB] failed to open '%s'", path.c_str());
    return points;
  }
