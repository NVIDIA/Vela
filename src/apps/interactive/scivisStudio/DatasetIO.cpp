...
const vsr::io::SubtreeArchiveContentDesc DATASET_DESC{
    DATASET_FILE_TYPE, DATASET_SCHEMA, vsr::io::ArchiveObjectPolicy::All};
...
void datasetMetadataToNode(const Dataset &dataset, vsr::core::DataNode &node)
{
...
bool nodeToDatasetMetadata(
    vsr::core::DataNode &node, Dataset &dataset, std::string *error)
{
  if (node.child("id"))
  ...
  if (auto *settings = node.child("importerSettings")) {
    settings->foreach_child([&](vsr::core::DataNode &entry) {
  ...
  if (dataset.sourceKind == DatasetSourceKind::Static) {
    ...
    if (auto *files = node.child("sourceFiles");
      ...
      files->foreach_child([&](vsr::core::DataNode &entry) {
...
// (ADR 0014).
bool datasetArchiveHasSceneRepresentation(vsr::core::DataNode &archive)
{
  bool hasContent = false;
  if (auto *objectDB = archive.child("objectDB")) {
    objectDB->foreach_child([&](vsr::core::DataNode &typeNode) {
...
vsr::scene::Volume *findDatasetVolume(vsr::scene::LayerNodeRef root)
{
  if (!root)
    return nullptr;
  vsr::scene::Volume *volume = nullptr;
  ...
  layer->traverse(root, [&](vsr::scene::LayerNode &node, int) {
    if (!volume && node->isObject() && node->type() == ANARI_VOLUME)
      volume = dynamic_cast<vsr::scene::Volume *>(node->getObject());
    ...
    std::string *error)
{
  ...
  animation.emplaceFileBinding<vsr::io::SpatialFieldFileBinding>(
...
bool materializeDeclaredDataset(vsr::scene::Scene &scene,
    vsr::animation::AnimationManager &animationManager,
    Dataset &&dataset,
    ...
    std::string *error)
{
  ...
  try {
    ...
    vsr::io::import_animations(scene,
        animationManager,
        {{vsr::io::ImporterType::VOLUME_ANIMATION, files}},
        root);
...
bool writeDatasetSourceListEdit(const std::filesystem::path &datasetFile,
    ...
    std::string *error)
{
  auto tree = std::make_shared<vsr::core::DataTree>();
    ...
    vsr::core::DataNode &archive)
{
  ...
  auto subtreeResult =
      vsr::io::validate_SubtreeArchiveContent(archive, DATASET_DESC);
  ...
  if (auto *animations = archive.child("animations")) {
    ...
    animations->foreach_child([&](vsr::core::DataNode &animation) {
    ...
    std::string *error)
{
  ...
  vsr::io::ArchivePlanOptions planOptions;
  planOptions.animationManager = &animationManager;
  planOptions.fileBindings = vsr::io::FileBindingArchivePolicy::Omit;
  const auto planResult =
      vsr::io::plan_SubtreeArchive(*scene, root, planOptions);
  ...
  vsr::core::DataTree tree;
  if (!vsr::io::serialize_SubtreeArchiveContent(
    ...
    std::string *error)
{
  ...
  vsr::io::SubtreeArchiveContentOptions options;
  options.animationManager = &animationManager;
  auto loadedContent = vsr::io::deserialize_SubtreeArchiveContent(
  ...
  if (validation.dataset.sourceKind == DatasetSourceKind::FileAnimation
      && !recreateFileAnimation(validation.dataset,
          ...
          loadedContent.root,
          error)) {
    vsr::io::rollback_SubtreeArchiveContent(
    ...
    vsr::scene::LayerNodeRef root)
{
  ...
  if (!result.accepted()) {
    vsr::core::logError("[removeDatasetRuntime] %s", result.message.c_str());
    return false;
  }
  ...
  std::vector<vsr::scene::Object *> pending;
    ...
    const vsr::scene::Object *object)
{
  if (!root || !object)
    return false;
  const auto result = vsr::io::plan_SubtreeArchive(scene, root);
