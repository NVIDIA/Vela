...
vsr::scene::LayerNodeRef resolveProjectAssetRoot(const vsr::scene::Scene &scene,
...
ProjectAssetWrite makePoolArchiveWrite(const vsr::scene::Scene &scene,
    ...
    std::string *error)
{
  ...
  // A legacy TSD project counts as an existing project here: Save As must not
  // scatter a second manifest over one.
  const auto manifest =
      resolveProjectFileForRead(request.directory / PROJECT_MANIFEST_FILENAME);
  ...
  for (size_t i = 0; i < result.project.cameraRigs.size(); ++i) {
    ...
    write.target = std::filesystem::path("cameras") / (savedRig.name + ".vsr");
    if (savingCurrent && !liveRig.persistedName.empty()) {
      write.ownedTarget =
          std::filesystem::path("cameras") / (liveRig.persistedName + ".vsr");
    }
  ...
  for (size_t i = 0; i < result.project.lightRigs.size(); ++i) {
    ...
    write.target = std::filesystem::path("lights") / (savedRig.name + ".vsr");
    if (savingCurrent && !liveRig.persistedName.empty()) {
      write.ownedTarget =
          std::filesystem::path("lights") / (liveRig.persistedName + ".vsr");
    }
    const auto expectedName = savedRig.name;
  ...
  auto manifestTree = std::make_shared<vsr::core::DataTree>();
  ...
  if (savingCurrent) {
    ...
    for (size_t i = 0; i < result.project.datasets.size(); ++i) {
      ...
      if (!oldName.empty()
        ...
        addRemoval(
            plan, std::filesystem::path("datasets") / (oldName + ".vsr"));
    ...
    for (size_t i = 0; i < result.project.cameraRigs.size(); ++i) {
      ...
      if (!oldName.empty()
        ...
        addRemoval(plan, std::filesystem::path("cameras") / (oldName + ".vsr"));
      }
    }
    for (size_t i = 0; i < result.project.lightRigs.size(); ++i) {
      ...
      if (!oldName.empty()
        ...
        addRemoval(plan, std::filesystem::path("lights") / (oldName + ".vsr"));
      }
    }
  }
