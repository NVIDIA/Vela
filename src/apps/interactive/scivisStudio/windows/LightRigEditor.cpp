...
int selectedLightIndex(const std::vector<vsr::scene::LayerNodeRef> &nodes,
    vsr::scene::LayerNodeRef selectedNode)
{
  if (!selectedNode)
    return -1;
...
void LightRigEditor::buildUI_lightList(LightRig &rig)
{
  ...
  m_selectedLight = selectedLightIndex(
      nodes, ctx ? ctx->getFirstSelected() : vsr::scene::LayerNodeRef{});
  ...
  auto selectedNode =
      hasSelection ? nodes[m_selectedLight] : vsr::scene::LayerNodeRef{};
