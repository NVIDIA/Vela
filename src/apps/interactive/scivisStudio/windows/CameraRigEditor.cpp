...
// Default rig Archives to a .vsr extension when the user didn't supply one.
std::string withVsrExtension(const std::string &path)
{
...
void CameraRigEditor::buildUI_rigControls()
{
  ...
  if (ImGui::Button("Load Archive...")) {
    ...
    m_app->getFilenameFromDialog(
        m_pendingFilename, vsr::ui::imgui::FileDialogMode::OpenFile);
  }
  ...
  if (ImGui::Button("Save Archive...")) {
    ...
    m_pendingFilename.clear();
    m_app->getFilenameFromDialog(
        m_pendingFilename, vsr::ui::imgui::FileDialogMode::SaveFile);
  }
    ...
    const char *label, ManipulatorState &state)
{
  ...
  if (ImGui::Checkbox("Fixed Distance", &hasFixedDistance)) {
    pose.fixedDist = hasFixedDistance ? pose.azeldist.z : vsr::math::inf;
    changed = true;
  }
...
void CameraRigEditor::buildUI_keyframes(CameraRig &cameraRig)
{
  ...
  vsr::ui::tooltipForPreviousItem("Set Rig View From Viewport");
  ImGui::EndDisabled();
  ...
  vsr::ui::tooltipForPreviousItem("Capture Keyframe At Current Frame");
  ImGui::EndDisabled();
  ...
  vsr::ui::tooltipForPreviousItem("Update Selected From Viewport");
  ImGui::EndDisabled();
  ...
  vsr::ui::tooltipForPreviousItem("Delete Keyframe");
  ImGui::EndDisabled();
...
void CameraRigEditor::pollPendingFileIO()
{
  ...
  if (request == PendingFileIO::Load) {
    ...
    if (!m_projectContext->saveCameraRigArchive(
            rigToSave, withVsrExtension(filename), &error)) {
      m_ioError = error;
      ImGui::OpenPopup("Camera Rig Archive Error");
    }
  }
