...
{
  // A legacy TSD project still spells its Dataset Archives ".tsd"; act on
  // whichever spelling is actually on disk.
  return resolveProjectFileForRead(
      project.projectDirectory / "datasets" / (dataset.persistedName + ".vsr"));
}

int commandFailed(const std::string &message)
{
  vsr::core::logError("[scivisStudioCLI] %s", message.c_str());
  return 1;
}

// One headless VSR context per command invocation, opened in bookkeeping
