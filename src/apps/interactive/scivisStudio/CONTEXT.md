...
**Dataset Archive**:
A portable native VSR representation of a Dataset without its project-local
...
and requires no external source I/O. It may contain ordinary core VSR
...
inventory. A generic VSR scene or another VSR Archive type is not a
dataset candidate.

**Camera Rig Archive**:
A portable native VSR representation of a camera rig.

**Light Rig Archive**:
A portable native VSR representation of a light rig.

**File Animation Dataset**:
A dataset that owns an authoritative ordered source-file list and uses it to
recreate one core VSR file animation together with its associated layer
...
**File Animation**:
A derived core VSR runtime animation that loads and unloads individual source
