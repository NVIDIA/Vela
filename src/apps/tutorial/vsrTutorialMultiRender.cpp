...
using float3 = vsr::math::float3;
using float4 = vsr::math::float4;
using uint2 = vsr::math::uint2;
...
int main()
{
  ...
  vsr::io::generate_randomSpheres(scene, scene.defaultLayer()->root(), true);
  ...
  auto *rIdx1 = ud.emplace<vsr::rendering::RenderIndexFlatRegistry>(
      scene, deviceName, device);
  auto *rIdx2 = ud.emplace<vsr::rendering::RenderIndexFlatRegistry>(
      scene, deviceName, device);
  ...
  // Mutate material
  scene.getObject<vsr::scene::Material>(0)->setParameter(
