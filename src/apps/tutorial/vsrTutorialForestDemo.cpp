...
template <typename T>
void print(vsr::core::Forest<T> &f)
{
  std::cout << "----Forest----\n";
...
int main()
{
  ...
  auto s = vsr::core::find_first_child(l1r, [](auto &v) { return v == 4; });
  ...
  int sum = 0;
  vsr::core::foreach_child(f.root(), [&](int &v) { sum += v; });
  std::cout << "sum of root node's children: " << sum << std::endl;

  sum = f.root()->value();
  vsr::core::forall_children(f.root(), [&](int &v) { sum += v; });
