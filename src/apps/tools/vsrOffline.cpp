...
struct Config
{
  vsr::math::float3 cameraPos = {0.f, 0.f, 3.f};
  vsr::math::float3 cameraLookAt = {0.f, 0.f, 0.f};
  vsr::math::float3 cameraUp = {0.f, 1.f, 0.f};
  float fovy = 40.f;
  ...
  vsr::math::float4 background = {0.05f, 0.05f, 0.05f, 1.f};
  ...
  {
    vsr::math::float3 direction;
    vsr::math::float3 color;
    float irradiance;
  };
...
static void printUsage(const char *programName)
{
  ...
  std::cout
      << "  -o, --output <file>        Output filename (default: vsrOffline.png)\n";
  std::cout
  ...
  std::cout << "  -vsr <file>                Load a VSR Scene Archive\n";
  std::cout << "\n";
...
static vsr::math::float3 parseFloat3(const char **argv, int &i)
{
  vsr::math::float3 result;
...
static std::vector<CameraChoice> collectSceneCameras()
{
  ...
  const auto &cameraDB = g_ctx->vsr.scene.objectDB().camera;

  vsr::core::foreach_item_const(cameraDB, [&](const auto *cam) {
    if (!cam)
      return;
...
{
  if (g_ctx->vsr.scene.numberOfObjects(ANARI_CAMERA) != 0)
    return;

  auto camera = g_ctx->vsr.scene.defaultCamera();
  if (camera) {
...
static bool resolveCameraSelectionFromCli(
    ...
    size_t &cameraIndex)
{
  size_t parsedIndex = VSR_INVALID_INDEX;
...
static bool resolvePromptSelection(const std::vector<CameraChoice> &cameras,
    ...
    size_t &cameraIndex)
{
  size_t parsedValue = VSR_INVALID_INDEX;
...
static void selectOfflineCamera()
{
  ...
  size_t selectedIndex = VSR_INVALID_INDEX;
  ...
  if (selectedIndex == VSR_INVALID_INDEX)
    std::exit(1);
...
static void configureDefaultSceneCameraPose()
{
  ...
  auto camera = g_ctx->vsr.scene.getObject<vsr::scene::Camera>(cameraIndex);
  if (!camera)
    return;

  auto pose = g_renderIndex->computeDefaultView();
  g_manipulator.setConfig(pose);
  vsr::rendering::updateCameraObject(*camera, g_manipulator);
}
...
static void setupSelectedSceneCamera()
{
  ...
  if (g_config.createdDefaultCamera) {
    ...
    auto camera = g_ctx->vsr.scene.getObject<vsr::scene::Camera>(
...
static void setupLights()
{
  ...
  for (const auto &lightConfig : g_config.directionalLights) {
    auto light =
        g_ctx->vsr.scene.createObject<vsr::scene::Light>("directional");
  ...
  if (g_config.directionalLights.empty()
      && g_ctx->vsr.scene.numberOfObjects(ANARI_LIGHT) == 0) {
    vsr::io::generate_default_lights(g_ctx->vsr.scene);
    printf("(added default light)...");
    fflush(stdout);
  } else {
    printf("(%zu light(s) in scene)...",
        g_ctx->vsr.scene.numberOfObjects(ANARI_LIGHT));
    fflush(stdout);
  }
...
static void renderFrames()
{
  ...
  int numFrames = 1;
  if (animMode) {
    auto &animMgr = g_ctx->vsr.animationMgr;
  ...
  for (int frameIndex = 0; frameIndex < numFrames; ++frameIndex) {
    ...
    if (animMode)
      g_ctx->vsr.animationMgr.setAnimationFrame(frameIndex);
