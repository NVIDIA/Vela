...
int main(int argc, const char *argv[])
{
  ...
  std::string warn;
  std::string err;
  std::string basePath = vsr::io::pathOf(inputFile);
  std::string file = vsr::io::fileOf(inputFile);
  ...
  std::vector<vsr::math::float3> v;
  std::vector<vsr::math::float3> vn;
  std::vector<vsr::math::float2> vt;
  ...
  for (auto &shape : objdata.shapes) {
    ...
    for (size_t i = 0; i < numIndices; i += 3) {
      ...
      auto vt0 = vti0 < 0
          ? vsr::math::float2(0.f)
          : vsr::math::float2(tc_in[vti0 * 2 + 0], tc_in[vti0 * 2 + 1]);
      auto vt1 = vti1 < 0
          ? vsr::math::float2(0.f)
          : vsr::math::float2(tc_in[vti1 * 2 + 0], tc_in[vti1 * 2 + 1]);
      auto vt2 = vti2 < 0
          ? vsr::math::float2(0.f)
          : vsr::math::float2(tc_in[vti2 * 2 + 0], tc_in[vti2 * 2 + 1]);
